...
class EnrichmentRecord(BaseModel):
    ...
    name: str = ""
    enrichment_type: EnrichmentType = EnrichmentType.SERVICE_CONTEXT
    enrichment_source: EnrichmentSource = EnrichmentSource.OTEL_SDK
    enrichment_quality: EnrichmentQuality = EnrichmentQuality.UNKNOWN
    score: float = 0.0
    service: str = ""
    team: str = ""
...
class DistributedTraceReport(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    total_records: int = 0
...
class DistributedTraceEnricher:
    """Distributed Trace Enricher — distributed trace enrichment and context propagation."""
    ...
    def __init__(
        ...
        logger.info(
            "distributed_trace_enricher.initialized",
    ...
    def record_entry(
        ...
        logger.info(
            "distributed_trace_enricher.entry_recorded",
            record_id=record.id,
            name=name,
    ...
    def get_record(self, record_id: str) -> EnrichmentRecord | None:
    ...
    def add_analysis(
        ...
        logger.info(
            "distributed_trace_enricher.analysis_added",
            name=name,
            enrichment_type=enrichment_type.value,
            analysis_score=analysis_score,
        )
    ...
    def generate_report(self) -> DistributedTraceReport:
        by_e1: dict[str, int] = {}
        by_e2: dict[str, int] = {}
        by_e3: dict[str, int] = {}
        for r in self._records:
            by_e1[r.enrichment_type.value] = by_e1.get(r.enrichment_type.value, 0) + 1
            by_e2[r.enrichment_source.value] = by_e2.get(r.enrichment_source.value, 0) + 1
            by_e3[r.enrichment_quality.value] = by_e3.get(r.enrichment_quality.value, 0) + 1
        ...
        if not recs:
            recs.append("Distributed Trace Enricher is healthy")
        return DistributedTraceReport(
            total_records=len(self._records),
            total_analyses=len(self._analyses),
            gap_count=gap_count,
            avg_score=avg_score,
            by_enrichment_type=by_e1,
            by_enrichment_source=by_e2,
            by_enrichment_quality=by_e3,
            top_gaps=top_gaps,
    ...
    def clear_data(self) -> dict[str, str]:
        ...
        logger.info("distributed_trace_enricher.cleared")
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "enrichment_type_distribution": e1_dist,
