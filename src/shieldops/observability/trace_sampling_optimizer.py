...
class SamplingEfficiency(StrEnum):
    OPTIMAL = "optimal"
...
class SamplingRecord(BaseModel):
    ...
    sampling_target: SamplingTarget = SamplingTarget.COST_REDUCTION
    sampling_efficiency: SamplingEfficiency = SamplingEfficiency.UNKNOWN
    score: float = 0.0
    service: str = ""
    team: str = ""
...
class TraceSamplingReport(BaseModel):
    ...
    by_sampling_target: dict[str, int] = Field(default_factory=dict)
    by_sampling_efficiency: dict[str, int] = Field(default_factory=dict)
...
class TraceSamplingOptimizer:
    ...
    def __init__(
        ...
        logger.info(
            "trace_sampling_optimizer.initialized",
    ...
    def record_entry(
        ...
        logger.info(
            "trace_sampling_optimizer.entry_recorded",
            record_id=record.id,
            name=name,
            sampling_strategy=sampling_strategy.value,
            sampling_target=sampling_target.value,
        )
        return record

    def get_record(self, record_id: str) -> SamplingRecord | None:
    ...
    def list_records(
        ...
        sampling_target: SamplingTarget | None = None,
        team: str | None = None,
        limit: int = 50,
    ) -> list[SamplingRecord]:
        ...
        if sampling_target is not None:
            results = [r for r in results if r.sampling_target == sampling_target]
        if team is not None:
    ...
    def add_analysis(
        ...
        logger.info(
            "trace_sampling_optimizer.analysis_added",
            name=name,
            sampling_strategy=sampling_strategy.value,
            analysis_score=analysis_score,
        )
    ...
    def generate_report(self) -> TraceSamplingReport:
        ...
        for r in self._records:
            ...
            by_e2[r.sampling_target.value] = by_e2.get(r.sampling_target.value, 0) + 1
            by_e3[r.sampling_efficiency.value] = by_e3.get(r.sampling_efficiency.value, 0) + 1
        ...
        if not recs:
            recs.append("Trace Sampling Optimizer is healthy")
        return TraceSamplingReport(
            ...
            by_sampling_target=by_e2,
            by_sampling_efficiency=by_e3,
            top_gaps=top_gaps,
    ...
    def clear_data(self) -> dict[str, str]:
        ...
        logger.info("trace_sampling_optimizer.cleared")
