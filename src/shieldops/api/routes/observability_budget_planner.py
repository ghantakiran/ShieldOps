"""Observability Budget Planner API routes."""
...
router = APIRouter(
    prefix="/observability-budget-planner",
...
def _get_engine() -> Any:
    if _engine is None:
        raise HTTPException(503, "Observability budget planner service unavailable")
    return _engine
...
async def get_budget(
    ...
    if result is None:
        raise HTTPException(404, f"Budget '{record_id}' not found")
    return result.model_dump()
...
async def identify_over_budget(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.identify_over_budget()


@router.get("/spend-rankings")
async def rank_by_spend(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.rank_by_spend()
