"""Cost Trend Forecaster API routes."""

from __future__ import annotations

from typing import Any

import structlog
from fastapi import APIRouter, Depends, HTTPException
from pydantic import BaseModel

from shieldops.api.auth.dependencies import require_role
from shieldops.billing.cost_trend import (
    CostCategory,
    ForecastConfidence,
    TrendDirection,
)

logger = structlog.get_logger()
router = APIRouter(prefix="/cost-trend", tags=["Cost Trend"])

_engine: Any = None


def set_engine(engine: Any) -> None:
    global _engine
    _engine = engine


def _get_engine() -> Any:
    if _engine is None:
        raise HTTPException(503, "Cost trend service unavailable")
    return _engine


class RecordTrendRequest(BaseModel):
    service: str
    cost_amount: float = 0.0
    direction: TrendDirection = TrendDirection.STABLE
    category: CostCategory = CostCategory.COMPUTE
    confidence: ForecastConfidence = ForecastConfidence.INSUFFICIENT_DATA
    period: str = ""
    details: str = ""
    model_config = {"extra": "forbid"}


class AddForecastRequest(BaseModel):
    service: str
    projected_cost: float = 0.0
    confidence: ForecastConfidence = ForecastConfidence.INSUFFICIENT_DATA
    period: str = ""
    growth_rate_pct: float = 0.0
    model_config = {"extra": "forbid"}


@router.post("/trends")
async def record_trend(
    body: RecordTrendRequest,
    _user: Any = Depends(require_role("operator")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    result = engine.record_trend(**body.model_dump())
    return result.model_dump()


@router.get("/trends")
async def list_trends(
    direction: TrendDirection | None = None,
    category: CostCategory | None = None,
    service: str | None = None,
    limit: int = 50,
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return [
        r.model_dump()
        for r in engine.list_trends(
            direction=direction, category=category, service=service, limit=limit
        )
    ]


@router.get("/trends/{record_id}")
async def get_trend(
    record_id: str,
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    result = engine.get_trend(record_id)
    if result is None:
        raise HTTPException(404, f"Cost trend record '{record_id}' not found")
    return result.model_dump()


@router.post("/forecasts")
async def add_forecast(
    body: AddForecastRequest,
    _user: Any = Depends(require_role("operator")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    result = engine.add_forecast(**body.model_dump())
    return result.model_dump()


@router.get("/by-category")
async def analyze_cost_by_category(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.analyze_cost_by_category()


@router.get("/high-growth")
async def identify_high_growth_services(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.identify_high_growth_services()


@router.get("/service-rankings")
async def rank_by_cost(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.rank_by_cost()


@router.get("/trends/detect")
async def detect_cost_trends(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.detect_cost_trends()


@router.get("/report")
async def generate_report(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.generate_report().model_dump()


@router.get("/stats")
async def get_stats(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.get_stats()


@router.post("/clear")
async def clear_data(
    _user: Any = Depends(require_role("operator")),  # type: ignore[arg-type]
) -> dict[str, str]:
    engine = _get_engine()
    return engine.clear_data()


ctf_route = router
