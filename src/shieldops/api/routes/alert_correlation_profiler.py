"""Alert Correlation Profiler API routes."""
...
router = APIRouter(
    prefix="/alert-correlation-profiler",
...
def _get_engine() -> Any:
    if _engine is None:
        raise HTTPException(503, "Alert correlation profiler service unavailable")
    return _engine
...
@router.get("/correlation-score-rankings")
async def rank_by_correlation_score(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.rank_by_correlation_score()
