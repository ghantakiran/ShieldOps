"""Dependency topology analyzer API routes."""
...
router = APIRouter(
    prefix="/dependency-topology",
    tags=["Dependency Topology"],
)
...
def _get_engine() -> Any:
    if _engine is None:
        raise HTTPException(
            503,
            "Dependency topology service unavailable",
        )
    return _engine
...
async def analyze_topology_health(
    service_name: str,
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.analyze_topology_health(service_name)


@router.get("/tightly-coupled")
async def identify_tightly_coupled(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.identify_tightly_coupled()


@router.get("/rankings")
async def rank_by_depth(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.rank_by_depth()


@router.get("/topology-risks")
async def detect_topology_risks(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.detect_topology_risks()
