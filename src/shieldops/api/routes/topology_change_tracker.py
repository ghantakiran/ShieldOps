"""Topology Change Tracker API routes."""
...
router = APIRouter(
    prefix="/topology-change-tracker",
...
def _get_engine() -> Any:
    if _engine is None:
        raise HTTPException(503, "Topology change tracker service unavailable")
    return _engine
...
async def get_change(
    ...
    if result is None:
        raise HTTPException(404, f"Change '{record_id}' not found")
    return result.model_dump()
...
async def analyze_change_distribution(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.analyze_change_distribution()
