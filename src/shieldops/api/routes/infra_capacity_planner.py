"""Infra capacity planner API routes."""

from __future__ import annotations

from typing import Any

import structlog
from fastapi import APIRouter, Depends, HTTPException
from pydantic import BaseModel

from shieldops.analytics.infra_capacity_planner import (
    CapacityAction,
    PlanningHorizon,
    ResourceType,
)
from shieldops.api.auth.dependencies import require_role

logger = structlog.get_logger()
router = APIRouter(
    prefix="/infra-capacity-planner",
    tags=["Infra Capacity Planner"],
)

_engine: Any = None


def set_engine(engine: Any) -> None:
    global _engine
    _engine = engine


def _get_engine() -> Any:
    if _engine is None:
        raise HTTPException(
            503,
            "Infra capacity planner unavailable",
        )
    return _engine


class RecordPlanRequest(BaseModel):
    service_name: str
    resource: ResourceType = ResourceType.COMPUTE
    horizon: PlanningHorizon = PlanningHorizon.MONTHLY
    action: CapacityAction = CapacityAction.MAINTAIN
    utilization_pct: float = 0.0
    details: str = ""


class AddRuleRequest(BaseModel):
    rule_name: str
    resource: ResourceType = ResourceType.COMPUTE
    horizon: PlanningHorizon = PlanningHorizon.MONTHLY
    target_utilization_pct: float = 70.0
    headroom_pct: float = 20.0


@router.post("/plans")
async def record_plan(
    body: RecordPlanRequest,
    _user: Any = Depends(require_role("operator")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    result = engine.record_plan(**body.model_dump())
    return result.model_dump()


@router.get("/plans")
async def list_plans(
    service_name: str | None = None,
    resource: ResourceType | None = None,
    limit: int = 50,
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return [
        r.model_dump()
        for r in engine.list_plans(
            service_name=service_name,
            resource=resource,
            limit=limit,
        )
    ]


@router.get("/plans/{record_id}")
async def get_plan(
    record_id: str,
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    result = engine.get_plan(record_id)
    if result is None:
        raise HTTPException(
            404,
            f"Plan '{record_id}' not found",
        )
    return result.model_dump()


@router.post("/rules")
async def add_rule(
    body: AddRuleRequest,
    _user: Any = Depends(require_role("operator")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    result = engine.add_rule(**body.model_dump())
    return result.model_dump()


@router.get("/efficiency/{service_name}")
async def analyze_capacity_efficiency(
    service_name: str,
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.analyze_capacity_efficiency(service_name)


@router.get("/over-provisioned")
async def identify_over_provisioned(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.identify_over_provisioned()


@router.get("/rankings")
async def rank_by_utilization(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.rank_by_utilization()


@router.get("/capacity-risks")
async def detect_capacity_risks(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.detect_capacity_risks()


@router.get("/report")
async def generate_report(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.generate_report().model_dump()


@router.get("/stats")
async def get_stats(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.get_stats()


@router.post("/clear")
async def clear_data(
    _user: Any = Depends(require_role("operator")),  # type: ignore[arg-type]
) -> dict[str, str]:
    engine = _get_engine()
    return engine.clear_data()


icp_route = router
