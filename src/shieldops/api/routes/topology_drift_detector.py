"""Topology Drift Detector API routes."""
...
router = APIRouter(
    prefix="/topology-drift-detector",
...
def _get_engine() -> Any:
    if _engine is None:
        raise HTTPException(503, "Topology drift detector service unavailable")
    return _engine
...
async def analyze_drift_distribution(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.analyze_drift_distribution()
