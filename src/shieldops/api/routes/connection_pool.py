"""Connection pool monitor API routes."""

from __future__ import annotations

from typing import Any

import structlog
from fastapi import APIRouter, Depends, HTTPException
from pydantic import BaseModel

from shieldops.analytics.connection_pool import (
    DatabaseType,
    PoolAction,
    PoolStatus,
)
from shieldops.api.auth.dependencies import require_role

logger = structlog.get_logger()
router = APIRouter(
    prefix="/connection-pool",
    tags=["Connection Pool"],
)

_engine: Any = None


def set_engine(engine: Any) -> None:
    global _engine
    _engine = engine


def _get_engine() -> Any:
    if _engine is None:
        raise HTTPException(503, "Connection pool service unavailable")
    return _engine


class RecordMetricsRequest(BaseModel):
    pool_name: str
    db_type: DatabaseType = DatabaseType.POSTGRESQL
    status: PoolStatus = PoolStatus.HEALTHY
    total_connections: int = 0
    active_connections: int = 0
    idle_connections: int = 0
    wait_time_ms: float = 0.0
    utilization_pct: float = 0.0
    details: str = ""


class AddRecommendationRequest(BaseModel):
    pool_name: str
    action: PoolAction = PoolAction.NO_ACTION
    reason: str = ""
    priority: int = 0


@router.post("/metrics")
async def record_metrics(
    body: RecordMetricsRequest,
    _user: Any = Depends(require_role("operator")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    result = engine.record_metrics(**body.model_dump())
    return result.model_dump()


@router.get("/metrics")
async def list_metrics(
    pool_name: str | None = None,
    db_type: DatabaseType | None = None,
    limit: int = 50,
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return [
        r.model_dump()
        for r in engine.list_metrics(pool_name=pool_name, db_type=db_type, limit=limit)
    ]


@router.get("/metrics/{record_id}")
async def get_metrics(
    record_id: str,
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    result = engine.get_metrics(record_id)
    if result is None:
        raise HTTPException(404, f"Pool metric record '{record_id}' not found")
    return result.model_dump()


@router.post("/recommendations")
async def add_recommendation(
    body: AddRecommendationRequest,
    _user: Any = Depends(require_role("operator")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    result = engine.add_recommendation(**body.model_dump())
    return result.model_dump()


@router.get("/health/{pool_name}")
async def analyze_pool_health(
    pool_name: str,
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.analyze_pool_health(pool_name)


@router.get("/saturated")
async def identify_saturated_pools(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.identify_saturated_pools()


@router.get("/leaks")
async def detect_potential_leaks(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.detect_potential_leaks()


@router.get("/wait-time")
async def rank_by_wait_time(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> list[dict[str, Any]]:
    engine = _get_engine()
    return engine.rank_by_wait_time()


@router.get("/report")
async def generate_report(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.generate_report().model_dump()


@router.get("/stats")
async def get_stats(
    _user: Any = Depends(require_role("viewer")),  # type: ignore[arg-type]
) -> dict[str, Any]:
    engine = _get_engine()
    return engine.get_stats()


@router.post("/clear")
async def clear_data(
    _user: Any = Depends(require_role("operator")),  # type: ignore[arg-type]
) -> dict[str, str]:
    engine = _get_engine()
    return engine.clear_data()


cpm_route = router
