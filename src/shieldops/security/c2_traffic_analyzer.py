"""C2 Traffic Analyzer — detect and analyze command-and-control traffic patterns."""

from __future__ import annotations

import time
import uuid
from enum import StrEnum
from typing import Any

import structlog
from pydantic import BaseModel, Field

logger = structlog.get_logger()


# --- Enums ---


class C2Protocol(StrEnum):
    HTTP_BEACON = "http_beacon"
    DNS_TUNNEL = "dns_tunnel"
    ICMP_COVERT = "icmp_covert"
    ENCRYPTED_CHANNEL = "encrypted_channel"
    SOCIAL_MEDIA = "social_media"


class TrafficPattern(StrEnum):
    PERIODIC = "periodic"
    JITTER = "jitter"
    BURST = "burst"
    LOW_SLOW = "low_slow"
    ADAPTIVE = "adaptive"


class C2Confidence(StrEnum):
    CONFIRMED = "confirmed"
    HIGH = "high"
    MEDIUM = "medium"
    LOW = "low"
    FALSE_POSITIVE = "false_positive"


# --- Models ---


class C2TrafficRecord(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    traffic_id: str = ""
    c2_protocol: C2Protocol = C2Protocol.HTTP_BEACON
    traffic_pattern: TrafficPattern = TrafficPattern.PERIODIC
    c2_confidence: C2Confidence = C2Confidence.CONFIRMED
    detection_score: float = 0.0
    service: str = ""
    team: str = ""
    created_at: float = Field(default_factory=time.time)


class C2TrafficAnalysis(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    traffic_id: str = ""
    c2_protocol: C2Protocol = C2Protocol.HTTP_BEACON
    analysis_score: float = 0.0
    threshold: float = 0.0
    breached: bool = False
    description: str = ""
    created_at: float = Field(default_factory=time.time)


class C2TrafficReport(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    total_records: int = 0
    total_analyses: int = 0
    gap_count: int = 0
    avg_detection_score: float = 0.0
    by_protocol: dict[str, int] = Field(default_factory=dict)
    by_pattern: dict[str, int] = Field(default_factory=dict)
    by_confidence: dict[str, int] = Field(default_factory=dict)
    top_gaps: list[str] = Field(default_factory=list)
    recommendations: list[str] = Field(default_factory=list)
    generated_at: float = Field(default_factory=time.time)
    created_at: float = Field(default_factory=time.time)


# --- Engine ---


class C2TrafficAnalyzer:
    """Detect and analyze C2 traffic patterns, protocols, and confidence levels."""

    def __init__(
        self,
        max_records: int = 200000,
        detection_threshold: float = 50.0,
    ) -> None:
        self._max_records = max_records
        self._detection_threshold = detection_threshold
        self._records: list[C2TrafficRecord] = []
        self._analyses: list[C2TrafficAnalysis] = []
        logger.info(
            "c2_traffic_analyzer.initialized",
            max_records=max_records,
            detection_threshold=detection_threshold,
        )

    # -- record / get / list ------------------------------------------------

    def record_traffic(
        self,
        traffic_id: str,
        c2_protocol: C2Protocol = C2Protocol.HTTP_BEACON,
        traffic_pattern: TrafficPattern = TrafficPattern.PERIODIC,
        c2_confidence: C2Confidence = C2Confidence.CONFIRMED,
        detection_score: float = 0.0,
        service: str = "",
        team: str = "",
    ) -> C2TrafficRecord:
        record = C2TrafficRecord(
            traffic_id=traffic_id,
            c2_protocol=c2_protocol,
            traffic_pattern=traffic_pattern,
            c2_confidence=c2_confidence,
            detection_score=detection_score,
            service=service,
            team=team,
        )
        self._records.append(record)
        if len(self._records) > self._max_records:
            self._records = self._records[-self._max_records :]
        logger.info(
            "c2_traffic_analyzer.traffic_recorded",
            record_id=record.id,
            traffic_id=traffic_id,
            c2_protocol=c2_protocol.value,
            traffic_pattern=traffic_pattern.value,
        )
        return record

    def get_traffic(self, record_id: str) -> C2TrafficRecord | None:
        for r in self._records:
            if r.id == record_id:
                return r
        return None

    def list_traffics(
        self,
        c2_protocol: C2Protocol | None = None,
        traffic_pattern: TrafficPattern | None = None,
        team: str | None = None,
        limit: int = 50,
    ) -> list[C2TrafficRecord]:
        results = list(self._records)
        if c2_protocol is not None:
            results = [r for r in results if r.c2_protocol == c2_protocol]
        if traffic_pattern is not None:
            results = [r for r in results if r.traffic_pattern == traffic_pattern]
        if team is not None:
            results = [r for r in results if r.team == team]
        return results[-limit:]

    def add_analysis(
        self,
        traffic_id: str,
        c2_protocol: C2Protocol = C2Protocol.HTTP_BEACON,
        analysis_score: float = 0.0,
        threshold: float = 0.0,
        breached: bool = False,
        description: str = "",
    ) -> C2TrafficAnalysis:
        analysis = C2TrafficAnalysis(
            traffic_id=traffic_id,
            c2_protocol=c2_protocol,
            analysis_score=analysis_score,
            threshold=threshold,
            breached=breached,
            description=description,
        )
        self._analyses.append(analysis)
        if len(self._analyses) > self._max_records:
            self._analyses = self._analyses[-self._max_records :]
        logger.info(
            "c2_traffic_analyzer.analysis_added",
            traffic_id=traffic_id,
            c2_protocol=c2_protocol.value,
            analysis_score=analysis_score,
        )
        return analysis

    # -- domain operations --------------------------------------------------

    def analyze_protocol_distribution(self) -> dict[str, Any]:
        """Group by c2_protocol; return count and avg detection_score."""
        protocol_data: dict[str, list[float]] = {}
        for r in self._records:
            key = r.c2_protocol.value
            protocol_data.setdefault(key, []).append(r.detection_score)
        result: dict[str, Any] = {}
        for protocol, scores in protocol_data.items():
            result[protocol] = {
                "count": len(scores),
                "avg_detection_score": round(sum(scores) / len(scores), 2),
            }
        return result

    def identify_detection_gaps(self) -> list[dict[str, Any]]:
        """Return records where detection_score < detection_threshold."""
        results: list[dict[str, Any]] = []
        for r in self._records:
            if r.detection_score < self._detection_threshold:
                results.append(
                    {
                        "record_id": r.id,
                        "traffic_id": r.traffic_id,
                        "c2_protocol": r.c2_protocol.value,
                        "detection_score": r.detection_score,
                        "service": r.service,
                        "team": r.team,
                    }
                )
        return sorted(results, key=lambda x: x["detection_score"])

    def rank_by_detection(self) -> list[dict[str, Any]]:
        """Group by service, avg detection_score, sort ascending (lowest first)."""
        svc_scores: dict[str, list[float]] = {}
        for r in self._records:
            svc_scores.setdefault(r.service, []).append(r.detection_score)
        results: list[dict[str, Any]] = []
        for svc, scores in svc_scores.items():
            results.append(
                {
                    "service": svc,
                    "avg_detection_score": round(sum(scores) / len(scores), 2),
                }
            )
        results.sort(key=lambda x: x["avg_detection_score"])
        return results

    def detect_detection_trends(self) -> dict[str, Any]:
        """Split-half comparison on analysis_score; delta threshold 5.0."""
        if len(self._analyses) < 2:
            return {"trend": "insufficient_data", "delta": 0.0}
        vals = [a.analysis_score for a in self._analyses]
        mid = len(vals) // 2
        first_half = vals[:mid]
        second_half = vals[mid:]
        avg_first = sum(first_half) / len(first_half)
        avg_second = sum(second_half) / len(second_half)
        delta = round(avg_second - avg_first, 2)
        if abs(delta) < 5.0:
            trend = "stable"
        elif delta > 0:
            trend = "improving"
        else:
            trend = "degrading"
        return {
            "trend": trend,
            "delta": delta,
            "avg_first_half": round(avg_first, 2),
            "avg_second_half": round(avg_second, 2),
        }

    # -- report / stats -----------------------------------------------------

    def generate_report(self) -> C2TrafficReport:
        by_protocol: dict[str, int] = {}
        by_pattern: dict[str, int] = {}
        by_confidence: dict[str, int] = {}
        for r in self._records:
            by_protocol[r.c2_protocol.value] = by_protocol.get(r.c2_protocol.value, 0) + 1
            by_pattern[r.traffic_pattern.value] = by_pattern.get(r.traffic_pattern.value, 0) + 1
            by_confidence[r.c2_confidence.value] = by_confidence.get(r.c2_confidence.value, 0) + 1
        gap_count = sum(1 for r in self._records if r.detection_score < self._detection_threshold)
        scores = [r.detection_score for r in self._records]
        avg_detection_score = round(sum(scores) / len(scores), 2) if scores else 0.0
        gap_list = self.identify_detection_gaps()
        top_gaps = [o["traffic_id"] for o in gap_list[:5]]
        recs: list[str] = []
        if self._records and gap_count > 0:
            recs.append(
                f"{gap_count} traffic record(s) below detection threshold "
                f"({self._detection_threshold})"
            )
        if self._records and avg_detection_score < self._detection_threshold:
            recs.append(
                f"Avg detection score {avg_detection_score} below threshold "
                f"({self._detection_threshold})"
            )
        if not recs:
            recs.append("C2 traffic analysis is healthy")
        return C2TrafficReport(
            total_records=len(self._records),
            total_analyses=len(self._analyses),
            gap_count=gap_count,
            avg_detection_score=avg_detection_score,
            by_protocol=by_protocol,
            by_pattern=by_pattern,
            by_confidence=by_confidence,
            top_gaps=top_gaps,
            recommendations=recs,
        )

    def clear_data(self) -> dict[str, str]:
        self._records.clear()
        self._analyses.clear()
        logger.info("c2_traffic_analyzer.cleared")
        return {"status": "cleared"}

    def get_stats(self) -> dict[str, Any]:
        protocol_dist: dict[str, int] = {}
        for r in self._records:
            key = r.c2_protocol.value
            protocol_dist[key] = protocol_dist.get(key, 0) + 1
        return {
            "total_records": len(self._records),
            "total_analyses": len(self._analyses),
            "detection_threshold": self._detection_threshold,
            "protocol_distribution": protocol_dist,
            "unique_teams": len({r.team for r in self._records}),
            "unique_services": len({r.service for r in self._records}),
        }
