"""Rollback Safety Engine — safe rollback planning and execution with risk assessment."""
...
class RollbackType(StrEnum):
    ...
    PERMISSION_REVERT = "permission_revert"
...
class RollbackRecord(BaseModel):
    ...
    name: str = ""
    rollback_type: RollbackType = RollbackType.CONFIG_RESTORE
    rollback_risk: RollbackRisk = RollbackRisk.MEDIUM
    rollback_status: RollbackStatus = RollbackStatus.PLANNED
    score: float = 0.0
    service: str = ""
    team: str = ""
...
class RollbackSafetyEngine:
    """Safe rollback planning and execution — risk analysis."""
    ...
    def __init__(
        ...
        logger.info(
            "rollback_safety_engine.initialized",
    ...
    def record_rollback(
        ...
        logger.info(
            "rollback_safety_engine.recorded",
            record_id=record.id,
            name=name,
    ...
    def list_records(
        ...
        rollback_risk: RollbackRisk | None = None,
        team: str | None = None,
        limit: int = 50,
    ) -> list[RollbackRecord]:
        ...
        if rollback_risk is not None:
            results = [r for r in results if r.rollback_risk == rollback_risk]
        if team is not None:
    ...
    def add_analysis(
        ...
        logger.info(
            "rollback_safety_engine.analysis_added",
            name=name,
            rollback_type=rollback_type.value,
            analysis_score=analysis_score,
        )
    ...
    def analyze_type_distribution(self) -> dict[str, Any]:
        """Group by rollback_type; return count and avg score."""
    ...
    def generate_report(self) -> RollbackReport:
        ...
        if self._records and gap_count > 0:
            recs.append(f"{gap_count} rollback(s) below threshold ({self._score_threshold})")
        ...
        if not recs:
            recs.append("Rollback safety metrics within healthy range")
    ...
    def clear_data(self) -> dict[str, str]:
        ...
        logger.info("rollback_safety_engine.cleared")
