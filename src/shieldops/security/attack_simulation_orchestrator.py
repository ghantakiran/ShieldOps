"""AttackSimulationOrchestrator — orchestrate attack simulations for security validation."""
...
class AttackSimulationType(StrEnum):
...
class AttackSimulationSource(StrEnum):
    SCANNER = "scanner"
    SIEM = "siem"
    EDR = "edr"
...
class AttackSimulationLevel(StrEnum):
    CRITICAL = "critical"
    HIGH = "high"
    MEDIUM = "medium"
    LOW = "low"
...
class AttackSimulationRecord(BaseModel):
    ...
    name: str = ""
    record_type: AttackSimulationType = AttackSimulationType.VULNERABILITY
    source: AttackSimulationSource = AttackSimulationSource.SCANNER
    level: AttackSimulationLevel = AttackSimulationLevel.MEDIUM
    score: float = 0.0
    service: str = ""
    team: str = ""
...
class AttackSimulationAnalysis(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    name: str = ""
    analysis_type: AttackSimulationType = AttackSimulationType.VULNERABILITY
...
class AttackSimulationOrchestrator:
    """Orchestrate attack simulations for security validation."""
    ...
    def __init__(
        ...
        logger.info(
            "attack_simulation_orchestrator.initialized",
    ...
    def record(
        ...
        logger.info(
            "attack_simulation_orchestrator.recorded",
            record_id=rec.id,
            name=name,
        )
        return rec
    ...
    def list_records(
        self,
        record_type: AttackSimulationType | None = None,
        source: AttackSimulationSource | None = None,
        team: str | None = None,
        limit: int = 50,
    ...
    def add_analysis(
        ...
        logger.info(
            "attack_simulation_orchestrator.analysis_added",
            name=name,
            analysis_score=analysis_score,
        )
        return analysis
    ...
    def generate_report(self) -> AttackSimulationReport:
        ...
        if not recs:
            recs.append("AttackSimulationOrchestrator posture is healthy")
    ...
    def clear_data(self) -> dict[str, str]:
        ...
        logger.info("attack_simulation_orchestrator.cleared")
