"""Threat Containment Validator — validate containment effectiveness and completeness."""
...
class ValidationRecord(BaseModel):
    ...
    name: str = ""
    validation_check: ValidationCheck = ValidationCheck.NETWORK_ISOLATED
    validation_result: ValidationResult = ValidationResult.PASSED
    validation_severity: ValidationSeverity = ValidationSeverity.MEDIUM
    score: float = 0.0
    service: str = ""
    team: str = ""
...
class ThreatContainmentValidator:
    """Validate containment effectiveness — isolation and cleanup."""
    ...
    def __init__(
        ...
        logger.info(
            "threat_containment_validator.initialized",
    ...
    def record_validation(
        ...
        logger.info(
            "threat_containment_validator.recorded",
            record_id=record.id,
            name=name,
    ...
    def add_analysis(
        ...
        logger.info(
            "threat_containment_validator.analysis_added",
            name=name,
            validation_check=validation_check.value,
            analysis_score=analysis_score,
        )
    ...
    def analyze_check_distribution(self) -> dict[str, Any]:
        """Group by validation_check; return count and avg score."""
        check_data: dict[str, list[float]] = {}
        for r in self._records:
            key = r.validation_check.value
            check_data.setdefault(key, []).append(r.score)
        result: dict[str, Any] = {}
        for k, scores in check_data.items():
            result[k] = {
                "count": len(scores),
    ...
    def generate_report(self) -> ValidationReport:
        ...
        for r in self._records:
            ...
            by_severity[r.validation_severity.value] = (
                by_severity.get(r.validation_severity.value, 0) + 1
            )
        ...
        if self._records and gap_count > 0:
            recs.append(f"{gap_count} validation(s) below threshold ({self._score_threshold})")
        ...
        if not recs:
            recs.append("Containment validation metrics within healthy range")
    ...
    def clear_data(self) -> dict[str, str]:
        ...
        logger.info("threat_containment_validator.cleared")
