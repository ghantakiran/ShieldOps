"""Continuous Identity Verifier — continuous identity verification and risk assessment."""

from __future__ import annotations

import time
import uuid
from enum import StrEnum
from typing import Any

import structlog
from pydantic import BaseModel, Field

logger = structlog.get_logger()


# --- Enums ---


class VerificationType(StrEnum):
    BIOMETRIC = "biometric"
    TOKEN = "token"  # noqa: S105
    CERTIFICATE = "certificate"
    BEHAVIORAL = "behavioral"
    CONTEXTUAL = "contextual"


class IdentityRisk(StrEnum):
    CRITICAL = "critical"
    HIGH = "high"
    MEDIUM = "medium"
    LOW = "low"
    TRUSTED = "trusted"


class VerificationStatus(StrEnum):
    VERIFIED = "verified"
    CHALLENGED = "challenged"
    DENIED = "denied"
    EXPIRED = "expired"
    PENDING = "pending"


# --- Models ---


class VerificationRecord(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    verification_id: str = ""
    verification_type: VerificationType = VerificationType.BIOMETRIC
    identity_risk: IdentityRisk = IdentityRisk.CRITICAL
    verification_status: VerificationStatus = VerificationStatus.VERIFIED
    confidence_score: float = 0.0
    service: str = ""
    team: str = ""
    created_at: float = Field(default_factory=time.time)


class VerificationAnalysis(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    verification_id: str = ""
    verification_type: VerificationType = VerificationType.BIOMETRIC
    analysis_score: float = 0.0
    threshold: float = 0.0
    breached: bool = False
    description: str = ""
    created_at: float = Field(default_factory=time.time)


class IdentityVerificationReport(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    total_records: int = 0
    total_analyses: int = 0
    gap_count: int = 0
    avg_confidence_score: float = 0.0
    by_verification_type: dict[str, int] = Field(default_factory=dict)
    by_identity_risk: dict[str, int] = Field(default_factory=dict)
    by_verification_status: dict[str, int] = Field(default_factory=dict)
    top_gaps: list[str] = Field(default_factory=list)
    recommendations: list[str] = Field(default_factory=list)
    generated_at: float = Field(default_factory=time.time)
    created_at: float = Field(default_factory=time.time)


# --- Engine ---


class ContinuousIdentityVerifier:
    """Continuous identity verification with risk scoring, anomaly detection, and trend analysis."""

    def __init__(
        self,
        max_records: int = 200000,
        verification_gap_threshold: float = 50.0,
    ) -> None:
        self._max_records = max_records
        self._verification_gap_threshold = verification_gap_threshold
        self._records: list[VerificationRecord] = []
        self._analyses: list[VerificationAnalysis] = []
        logger.info(
            "continuous_identity_verifier.initialized",
            max_records=max_records,
            verification_gap_threshold=verification_gap_threshold,
        )

    # -- record / get / list ------------------------------------------------

    def record_verification(
        self,
        verification_id: str,
        verification_type: VerificationType = VerificationType.BIOMETRIC,
        identity_risk: IdentityRisk = IdentityRisk.CRITICAL,
        verification_status: VerificationStatus = VerificationStatus.VERIFIED,
        confidence_score: float = 0.0,
        service: str = "",
        team: str = "",
    ) -> VerificationRecord:
        record = VerificationRecord(
            verification_id=verification_id,
            verification_type=verification_type,
            identity_risk=identity_risk,
            verification_status=verification_status,
            confidence_score=confidence_score,
            service=service,
            team=team,
        )
        self._records.append(record)
        if len(self._records) > self._max_records:
            self._records = self._records[-self._max_records :]
        logger.info(
            "continuous_identity_verifier.verification_recorded",
            record_id=record.id,
            verification_id=verification_id,
            verification_type=verification_type.value,
            identity_risk=identity_risk.value,
        )
        return record

    def get_verification(self, record_id: str) -> VerificationRecord | None:
        for r in self._records:
            if r.id == record_id:
                return r
        return None

    def list_verifications(
        self,
        verification_type: VerificationType | None = None,
        identity_risk: IdentityRisk | None = None,
        team: str | None = None,
        limit: int = 50,
    ) -> list[VerificationRecord]:
        results = list(self._records)
        if verification_type is not None:
            results = [r for r in results if r.verification_type == verification_type]
        if identity_risk is not None:
            results = [r for r in results if r.identity_risk == identity_risk]
        if team is not None:
            results = [r for r in results if r.team == team]
        return results[-limit:]

    def add_analysis(
        self,
        verification_id: str,
        verification_type: VerificationType = VerificationType.BIOMETRIC,
        analysis_score: float = 0.0,
        threshold: float = 0.0,
        breached: bool = False,
        description: str = "",
    ) -> VerificationAnalysis:
        analysis = VerificationAnalysis(
            verification_id=verification_id,
            verification_type=verification_type,
            analysis_score=analysis_score,
            threshold=threshold,
            breached=breached,
            description=description,
        )
        self._analyses.append(analysis)
        if len(self._analyses) > self._max_records:
            self._analyses = self._analyses[-self._max_records :]
        logger.info(
            "continuous_identity_verifier.analysis_added",
            verification_id=verification_id,
            verification_type=verification_type.value,
            analysis_score=analysis_score,
        )
        return analysis

    # -- domain operations --------------------------------------------------

    def analyze_verification_distribution(self) -> dict[str, Any]:
        """Group by verification_type; return count and avg confidence_score."""
        type_data: dict[str, list[float]] = {}
        for r in self._records:
            key = r.verification_type.value
            type_data.setdefault(key, []).append(r.confidence_score)
        result: dict[str, Any] = {}
        for vtype, scores in type_data.items():
            result[vtype] = {
                "count": len(scores),
                "avg_confidence_score": round(sum(scores) / len(scores), 2),
            }
        return result

    def identify_verification_gaps(self) -> list[dict[str, Any]]:
        """Return records where confidence_score < verification_gap_threshold."""
        results: list[dict[str, Any]] = []
        for r in self._records:
            if r.confidence_score < self._verification_gap_threshold:
                results.append(
                    {
                        "record_id": r.id,
                        "verification_id": r.verification_id,
                        "verification_type": r.verification_type.value,
                        "confidence_score": r.confidence_score,
                        "service": r.service,
                        "team": r.team,
                    }
                )
        return sorted(results, key=lambda x: x["confidence_score"])

    def rank_by_verification(self) -> list[dict[str, Any]]:
        """Group by service, avg confidence_score, sort ascending (lowest first)."""
        svc_scores: dict[str, list[float]] = {}
        for r in self._records:
            svc_scores.setdefault(r.service, []).append(r.confidence_score)
        results: list[dict[str, Any]] = []
        for svc, scores in svc_scores.items():
            results.append(
                {
                    "service": svc,
                    "avg_confidence_score": round(sum(scores) / len(scores), 2),
                }
            )
        results.sort(key=lambda x: x["avg_confidence_score"])
        return results

    def detect_verification_trends(self) -> dict[str, Any]:
        """Split-half comparison on analysis_score; delta threshold 5.0."""
        if len(self._analyses) < 2:
            return {"trend": "insufficient_data", "delta": 0.0}
        vals = [a.analysis_score for a in self._analyses]
        mid = len(vals) // 2
        first_half = vals[:mid]
        second_half = vals[mid:]
        avg_first = sum(first_half) / len(first_half)
        avg_second = sum(second_half) / len(second_half)
        delta = round(avg_second - avg_first, 2)
        if abs(delta) < 5.0:
            trend = "stable"
        elif delta > 0:
            trend = "improving"
        else:
            trend = "degrading"
        return {
            "trend": trend,
            "delta": delta,
            "avg_first_half": round(avg_first, 2),
            "avg_second_half": round(avg_second, 2),
        }

    # -- report / stats -----------------------------------------------------

    def generate_report(self) -> IdentityVerificationReport:
        by_verification_type: dict[str, int] = {}
        by_identity_risk: dict[str, int] = {}
        by_verification_status: dict[str, int] = {}
        for r in self._records:
            by_verification_type[r.verification_type.value] = (
                by_verification_type.get(r.verification_type.value, 0) + 1
            )
            by_identity_risk[r.identity_risk.value] = (
                by_identity_risk.get(r.identity_risk.value, 0) + 1
            )
            by_verification_status[r.verification_status.value] = (
                by_verification_status.get(r.verification_status.value, 0) + 1
            )
        gap_count = sum(
            1 for r in self._records if r.confidence_score < self._verification_gap_threshold
        )
        scores = [r.confidence_score for r in self._records]
        avg_confidence_score = round(sum(scores) / len(scores), 2) if scores else 0.0
        gap_list = self.identify_verification_gaps()
        top_gaps = [o["verification_id"] for o in gap_list[:5]]
        recs: list[str] = []
        if self._records and gap_count > 0:
            recs.append(
                f"{gap_count} verification(s) below confidence threshold "
                f"({self._verification_gap_threshold})"
            )
        if self._records and avg_confidence_score < self._verification_gap_threshold:
            recs.append(
                f"Avg confidence score {avg_confidence_score} below threshold "
                f"({self._verification_gap_threshold})"
            )
        if not recs:
            recs.append("Continuous identity verification is healthy")
        return IdentityVerificationReport(
            total_records=len(self._records),
            total_analyses=len(self._analyses),
            gap_count=gap_count,
            avg_confidence_score=avg_confidence_score,
            by_verification_type=by_verification_type,
            by_identity_risk=by_identity_risk,
            by_verification_status=by_verification_status,
            top_gaps=top_gaps,
            recommendations=recs,
        )

    def clear_data(self) -> dict[str, str]:
        self._records.clear()
        self._analyses.clear()
        logger.info("continuous_identity_verifier.cleared")
        return {"status": "cleared"}

    def get_stats(self) -> dict[str, Any]:
        type_dist: dict[str, int] = {}
        for r in self._records:
            key = r.verification_type.value
            type_dist[key] = type_dist.get(key, 0) + 1
        return {
            "total_records": len(self._records),
            "total_analyses": len(self._analyses),
            "verification_gap_threshold": self._verification_gap_threshold,
            "verification_type_distribution": type_dist,
            "unique_teams": len({r.team for r in self._records}),
            "unique_services": len({r.service for r in self._records}),
        }
