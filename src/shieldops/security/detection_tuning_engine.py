"""Detection Tuning Engine — optimize detection rules for accuracy and performance."""
...
class TuningAction(StrEnum):
    ...
    CORRELATION_ADD = "correlation_add"
...
class TuningRecord(BaseModel):
    ...
    rule_name: str = ""
    tuning_action: TuningAction = TuningAction.THRESHOLD_ADJUST
    detection_type: DetectionType = DetectionType.SIGNATURE
    tuning_impact: TuningImpact = TuningImpact.MEDIUM
...
class DetectionTuningEngine:
    """Optimize detection rules for accuracy and performance through tuning analysis."""
    ...
    def __init__(
        ...
        logger.info(
            "detection_tuning_engine.initialized",
    ...
    def record_tuning(
        ...
        logger.info(
            "detection_tuning_engine.tuning_recorded",
            record_id=record.id,
            rule_name=rule_name,
            tuning_action=tuning_action.value,
    ...
    def list_records(
        self,
        tuning_action: TuningAction | None = None,
        detection_type: DetectionType | None = None,
        team: str | None = None,
        limit: int = 50,
    ) -> list[TuningRecord]:
        results = list(self._records)
        if tuning_action is not None:
            results = [r for r in results if r.tuning_action == tuning_action]
    ...
    def add_analysis(
        ...
        logger.info(
            "detection_tuning_engine.analysis_added",
            rule_name=rule_name,
            tuning_action=tuning_action.value,
            analysis_score=analysis_score,
        )
    ...
    def analyze_distribution(self) -> dict[str, Any]:
        """Group by tuning_action; return count and avg effectiveness_score."""
        action_data: dict[str, list[float]] = {}
        for r in self._records:
            key = r.tuning_action.value
            action_data.setdefault(key, []).append(r.effectiveness_score)
        result: dict[str, Any] = {}
                ...
                results.append(
                    {
                        ...
                        "tuning_action": r.tuning_action.value,
    ...
    def generate_report(self) -> TuningReport:
        ...
        for r in self._records:
            by_action[r.tuning_action.value] = by_action.get(r.tuning_action.value, 0) + 1
            by_type[r.detection_type.value] = by_type.get(r.detection_type.value, 0) + 1
            by_impact[r.tuning_impact.value] = by_impact.get(r.tuning_impact.value, 0) + 1
        ...
        if self._records and avg_score < self._threshold:
            recs.append(f"Avg effectiveness score {avg_score} below threshold ({self._threshold})")
        if not recs:
            recs.append("Detection tuning is healthy")
        return TuningReport(
    ...
    def clear_data(self) -> dict[str, str]:
        ...
        logger.info("detection_tuning_engine.cleared")
