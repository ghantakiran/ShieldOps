"""Unified Detection Engine — unified detection engine across multiple security data sources."""
...
class DetectionRecord(BaseModel):
    ...
    detection_confidence: DetectionConfidence = DetectionConfidence.INFORMATIONAL
    score: float = 0.0
    service: str = ""
    team: str = ""
...
class UnifiedDetectionReport(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    total_records: int = 0
    total_analyses: int = 0
    gap_count: int = 0
    avg_score: float = 0.0
    by_detection_type: dict[str, int] = Field(default_factory=dict)
...
class UnifiedDetectionEngine:
    ...
    def __init__(
        ...
        self._analyses: list[DetectionAnalysis] = []
        logger.info(
            "unified_detection_engine.initialized",
    ...
    def record_entry(
        ...
        logger.info(
            "unified_detection_engine.entry_recorded",
            record_id=record.id,
            name=name,
    ...
    def get_record(self, record_id: str) -> DetectionRecord | None:
    ...
    def add_analysis(
        ...
        logger.info(
            "unified_detection_engine.analysis_added",
            name=name,
            detection_type=detection_type.value,
            analysis_score=analysis_score,
        )
    ...
    def generate_report(self) -> UnifiedDetectionReport:
        by_e1: dict[str, int] = {}
        by_e2: dict[str, int] = {}
        by_e3: dict[str, int] = {}
        for r in self._records:
            by_e1[r.detection_type.value] = by_e1.get(r.detection_type.value, 0) + 1
            by_e2[r.detection_domain.value] = by_e2.get(r.detection_domain.value, 0) + 1
        ...
        if not recs:
            recs.append("Unified Detection Engine is healthy")
        return UnifiedDetectionReport(
            total_records=len(self._records),
            total_analyses=len(self._analyses),
            gap_count=gap_count,
            avg_score=avg_score,
            by_detection_type=by_e1,
            by_detection_domain=by_e2,
    ...
    def clear_data(self) -> dict[str, str]:
        ...
        logger.info("unified_detection_engine.cleared")
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "detection_type_distribution": e1_dist,
