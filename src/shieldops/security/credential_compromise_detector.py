"""CredentialCompromiseDetector — detect credential compromise through behavioral analysis."""
...
class CredentialRecord(BaseModel):
    ...
    name: str = ""
    record_type: CredentialType = CredentialType.VULNERABILITY
    source: CredentialSource = CredentialSource.SCANNER
    level: CredentialLevel = CredentialLevel.MEDIUM
    score: float = 0.0
    service: str = ""
    team: str = ""
...
class CredentialAnalysis(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    name: str = ""
    analysis_type: CredentialType = CredentialType.VULNERABILITY
...
class CredentialCompromiseDetector:
    """Detect credential compromise through behavioral analysis."""
    ...
    def __init__(
        ...
        self._analyses: list[CredentialAnalysis] = []
        logger.info(
            "credential_compromise_detector.initialized",
    ...
    def record(
        ...
        logger.info(
            "credential_compromise_detector.recorded",
            record_id=rec.id,
            name=name,
        )
        return rec

    def get_record(self, record_id: str) -> CredentialRecord | None:
    ...
    def list_records(
        self,
        record_type: CredentialType | None = None,
    ...
    def add_analysis(
        ...
        logger.info(
            "credential_compromise_detector.analysis_added",
            name=name,
            analysis_score=analysis_score,
        )
        return analysis
    ...
    def generate_report(self) -> CredentialReport:
        ...
        if not recs:
            recs.append("CredentialCompromiseDetector posture is healthy")
    ...
    def clear_data(self) -> dict[str, str]:
        ...
        logger.info("credential_compromise_detector.cleared")
