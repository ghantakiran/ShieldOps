...
class DriftRecord(BaseModel):
    ...
    drift_source: DriftSource = DriftSource.TERRAFORM_STATE
    drift_severity: DriftSeverity = DriftSeverity.COSMETIC
    score: float = 0.0
    service: str = ""
    team: str = ""
...
class InfrastructureDriftIntelligence:
    """Infrastructure Drift Intelligence — infrastructure drift intelligence and remediation."""
    ...
    def __init__(
        ...
        logger.info(
            "infrastructure_drift_intelligence.initialized",
    ...
    def record_entry(
        ...
        logger.info(
            "infrastructure_drift_intelligence.entry_recorded",
            record_id=record.id,
            name=name,
            drift_type=drift_type.value,
            drift_source=drift_source.value,
        )
        return record
    ...
    def list_records(
        ...
        drift_source: DriftSource | None = None,
        team: str | None = None,
        limit: int = 50,
    ) -> list[DriftRecord]:
        ...
        if drift_source is not None:
            results = [r for r in results if r.drift_source == drift_source]
        if team is not None:
    ...
    def add_analysis(
        ...
        logger.info(
            "infrastructure_drift_intelligence.analysis_added",
            name=name,
            drift_type=drift_type.value,
            analysis_score=analysis_score,
        )
    ...
    def generate_report(self) -> InfrastructureDriftReport:
        ...
        for r in self._records:
            ...
            by_e2[r.drift_source.value] = by_e2.get(r.drift_source.value, 0) + 1
            by_e3[r.drift_severity.value] = by_e3.get(r.drift_severity.value, 0) + 1
        ...
        if not recs:
            recs.append("Infrastructure Drift Intelligence is healthy")
        return InfrastructureDriftReport(
            ...
            avg_score=avg_score,
            by_drift_type=by_e1,
            by_drift_source=by_e2,
            by_drift_severity=by_e3,
            top_gaps=top_gaps,
    ...
    def clear_data(self) -> dict[str, str]:
        ...
        logger.info("infrastructure_drift_intelligence.cleared")
