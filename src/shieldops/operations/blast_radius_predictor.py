"""Blast Radius Predictor — predict impact scope of failures before execution."""
...
class BlastRadiusPrediction(BaseModel):
    ...
    impact_type: ImpactType = ImpactType.SERVICE_DEGRADATION
    radius_scope: RadiusScope = RadiusScope.SINGLE_SERVICE
...
class BlastRadiusPredictor:
    """Predict and bound the blast radius of failure scenarios pre-execution."""
    ...
    def __init__(
        ...
        self._records: list[BlastRadiusPrediction] = []
        self._analyses: list[RadiusAnalysis] = []
        logger.info(
            "blast_radius_predictor.initialized",
    ...
    def record_prediction(
        ...
        logger.info(
            "blast_radius_predictor.prediction_recorded",
            record_id=record.id,
            service=service,
    ...
    def get_prediction(self, record_id: str) -> BlastRadiusPrediction | None:
    ...
    def add_analysis(
        ...
        logger.info(
            "blast_radius_predictor.analysis_added",
            impact_type=impact_type.value,
            analysis_score=analysis_score,
        )
        return analysis
    ...
    def analyze_distribution(self) -> dict[str, Any]:
        """Group by impact_type; return count and avg score."""
        type_data: dict[str, list[float]] = {}
        for r in self._records:
            key = r.impact_type.value
    ...
    def identify_radius_gaps(self) -> list[dict[str, Any]]:
    ...
    def generate_report(self) -> BlastRadiusReport:
        ...
        gap_list = self.identify_radius_gaps()
        top_gaps = [o["service"] for o in gap_list[:5]]
        recs: list[str] = []
        if self._records and gap_count > 0:
            recs.append(
                f"{gap_count} service(s) with high blast radius (threshold {self._threshold})"
            )
        if self._records and avg_score < self._threshold:
            recs.append(f"Avg score {avg_score} below threshold ({self._threshold})")
        if not recs:
            recs.append("Blast radius predictions indicate healthy containment")
    ...
    def clear_data(self) -> dict[str, str]:
        ...
        logger.info("blast_radius_predictor.cleared")
