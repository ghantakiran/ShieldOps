"""
Tests for shieldops.compliance.security_control_automation_scorer — SecurityControlAutomationScorer.
"""

from __future__ import annotations

from shieldops.compliance.security_control_automation_scorer import (
    SecurityControlAnalysis,
...
def _engine(**kw) -> SecurityControlAutomationScorer:
    return SecurityControlAutomationScorer(**kw)


class TestEnums:
    def test_type_control(self):
        assert SecurityControlType.CONTROL == "control"

    def test_type_policy(self):
        assert SecurityControlType.POLICY == "policy"

    def test_type_regulation(self):
        assert SecurityControlType.REGULATION == "regulation"

    def test_type_standard(self):
        assert SecurityControlType.STANDARD == "standard"

    def test_type_framework(self):
        assert SecurityControlType.FRAMEWORK == "framework"

    def test_source_audit(self):
        assert SecurityControlSource.AUDIT == "audit"

    def test_source_automated_scan(self):
        assert SecurityControlSource.AUTOMATED_SCAN == "automated_scan"

    def test_source_manual_review(self):
        assert SecurityControlSource.MANUAL_REVIEW == "manual_review"

    def test_source_continuous_monitor(self):
        assert SecurityControlSource.CONTINUOUS_MONITOR == "continuous_monitor"

    def test_source_third_party(self):
        assert SecurityControlSource.THIRD_PARTY == "third_party"

    def test_level_compliant(self):
        assert SecurityControlLevel.COMPLIANT == "compliant"

    def test_level_partial(self):
        assert SecurityControlLevel.PARTIAL == "partial"

    def test_level_non_compliant(self):
        assert SecurityControlLevel.NON_COMPLIANT == "non_compliant"

    def test_level_not_assessed(self):
        assert SecurityControlLevel.NOT_ASSESSED == "not_assessed"

    def test_level_exempt(self):
        assert SecurityControlLevel.EXEMPT == "exempt"


class TestModels:
    def test_record_defaults(self):
        r = SecurityControlRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == SecurityControlType.CONTROL
        assert r.source == SecurityControlSource.AUDIT
        assert r.level == SecurityControlLevel.NON_COMPLIANT
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = SecurityControlAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == SecurityControlType.CONTROL
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=SecurityControlType.CONTROL)
        e.record("b", record_type=SecurityControlType.POLICY)
        results = e.list_records(record_type=SecurityControlType.CONTROL)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=SecurityControlSource.AUDIT)
        e.record("b", source=SecurityControlSource.AUTOMATED_SCAN)
        results = e.list_records(source=SecurityControlSource.AUDIT)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = SecurityControlType.CONTROL
        t2 = SecurityControlType.POLICY
