"""Tests for shieldops.security.playbook_execution_engine — PlaybookExecutionEngine."""

from __future__ import annotations

from shieldops.security.playbook_execution_engine import (
    PlaybookAnalysis,
...
def _engine(**kw) -> PlaybookExecutionEngine:
    return PlaybookExecutionEngine(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert PlaybookType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert PlaybookType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert PlaybookType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert PlaybookType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert PlaybookType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert PlaybookSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert PlaybookSource.SIEM == "siem"

    def test_source_edr(self):
        assert PlaybookSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert PlaybookSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert PlaybookSource.MANUAL == "manual"

    def test_level_critical(self):
        assert PlaybookLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert PlaybookLevel.HIGH == "high"

    def test_level_medium(self):
        assert PlaybookLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert PlaybookLevel.LOW == "low"

    def test_level_info(self):
        assert PlaybookLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == PlaybookType.VULNERABILITY
        assert r.source == PlaybookSource.SCANNER
        assert r.level == PlaybookLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == PlaybookType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=PlaybookType.VULNERABILITY)
        e.record("b", record_type=PlaybookType.MISCONFIGURATION)
        results = e.list_records(record_type=PlaybookType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=PlaybookSource.SCANNER)
        e.record("b", source=PlaybookSource.SIEM)
        results = e.list_records(source=PlaybookSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = PlaybookType.VULNERABILITY
        t2 = PlaybookType.MISCONFIGURATION
