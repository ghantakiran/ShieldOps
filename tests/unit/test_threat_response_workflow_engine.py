"""Tests for shieldops.security.threat_response_workflow_engine — ThreatResponseWorkflowEngine."""

from __future__ import annotations

from shieldops.security.threat_response_workflow_engine import (
    ThreatResponseAnalysis,
    ThreatResponseLevel,
    ThreatResponseRecord,
    ThreatResponseReport,
    ThreatResponseSource,
    ThreatResponseType,
    ThreatResponseWorkflowEngine,
)


def _engine(**kw) -> ThreatResponseWorkflowEngine:
    return ThreatResponseWorkflowEngine(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert ThreatResponseType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert ThreatResponseType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert ThreatResponseType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert ThreatResponseType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert ThreatResponseType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert ThreatResponseSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert ThreatResponseSource.SIEM == "siem"

    def test_source_edr(self):
        assert ThreatResponseSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert ThreatResponseSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert ThreatResponseSource.MANUAL == "manual"

    def test_level_critical(self):
        assert ThreatResponseLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert ThreatResponseLevel.HIGH == "high"

    def test_level_medium(self):
        assert ThreatResponseLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert ThreatResponseLevel.LOW == "low"

    def test_level_info(self):
        assert ThreatResponseLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == ThreatResponseType.VULNERABILITY
        assert r.source == ThreatResponseSource.SCANNER
        assert r.level == ThreatResponseLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = ThreatResponseAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == ThreatResponseType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=ThreatResponseType.VULNERABILITY)
        e.record("b", record_type=ThreatResponseType.MISCONFIGURATION)
        results = e.list_records(record_type=ThreatResponseType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=ThreatResponseSource.SCANNER)
        e.record("b", source=ThreatResponseSource.SIEM)
        results = e.list_records(source=ThreatResponseSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = ThreatResponseType.VULNERABILITY
        t2 = ThreatResponseType.MISCONFIGURATION
