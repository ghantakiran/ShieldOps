"""Tests for shieldops.analytics.mean_time_to_contain_optimizer — MeanTimeToContainOptimizer."""

from __future__ import annotations

from shieldops.analytics.mean_time_to_contain_optimizer import (
    MeanTimeToAnalysis,
...
def _engine(**kw) -> MeanTimeToContainOptimizer:
    return MeanTimeToContainOptimizer(**kw)


class TestEnums:
    def test_type_metric(self):
        assert MeanTimeToType.METRIC == "metric"

    def test_type_trend(self):
        assert MeanTimeToType.TREND == "trend"

    def test_type_forecast(self):
        assert MeanTimeToType.FORECAST == "forecast"

    def test_type_anomaly(self):
        assert MeanTimeToType.ANOMALY == "anomaly"

    def test_type_benchmark(self):
        assert MeanTimeToType.BENCHMARK == "benchmark"

    def test_source_telemetry(self):
        assert MeanTimeToSource.TELEMETRY == "telemetry"

    def test_source_log_analysis(self):
        assert MeanTimeToSource.LOG_ANALYSIS == "log_analysis"

    def test_source_apm(self):
        assert MeanTimeToSource.APM == "apm"

    def test_source_custom(self):
        assert MeanTimeToSource.CUSTOM == "custom"

    def test_source_synthetic(self):
        assert MeanTimeToSource.SYNTHETIC == "synthetic"

    def test_level_critical(self):
        assert MeanTimeToLevel.CRITICAL == "critical"

    def test_level_warning(self):
        assert MeanTimeToLevel.WARNING == "warning"

    def test_level_normal(self):
        assert MeanTimeToLevel.NORMAL == "normal"

    def test_level_optimal(self):
        assert MeanTimeToLevel.OPTIMAL == "optimal"

    def test_level_unknown(self):
        assert MeanTimeToLevel.UNKNOWN == "unknown"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == MeanTimeToType.METRIC
        assert r.source == MeanTimeToSource.TELEMETRY
        assert r.level == MeanTimeToLevel.NORMAL
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = MeanTimeToAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == MeanTimeToType.METRIC
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=MeanTimeToType.METRIC)
        e.record("b", record_type=MeanTimeToType.TREND)
        results = e.list_records(record_type=MeanTimeToType.METRIC)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=MeanTimeToSource.TELEMETRY)
        e.record("b", source=MeanTimeToSource.LOG_ANALYSIS)
        results = e.list_records(source=MeanTimeToSource.TELEMETRY)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = MeanTimeToType.METRIC
        t2 = MeanTimeToType.TREND
