"""Tests for shieldops.security.identity_attack_path_analyzer — IdentityAttackPathAnalyzer."""

from __future__ import annotations

from shieldops.security.identity_attack_path_analyzer import (
    IdentityAttackAnalysis,
    IdentityAttackLevel,
    IdentityAttackPathAnalyzer,
    IdentityAttackRecord,
    IdentityAttackReport,
    IdentityAttackSource,
    IdentityAttackType,
)


def _engine(**kw) -> IdentityAttackPathAnalyzer:
    return IdentityAttackPathAnalyzer(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert IdentityAttackType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert IdentityAttackType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert IdentityAttackType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert IdentityAttackType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert IdentityAttackType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert IdentityAttackSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert IdentityAttackSource.SIEM == "siem"

    def test_source_edr(self):
        assert IdentityAttackSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert IdentityAttackSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert IdentityAttackSource.MANUAL == "manual"

    def test_level_critical(self):
        assert IdentityAttackLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert IdentityAttackLevel.HIGH == "high"

    def test_level_medium(self):
        assert IdentityAttackLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert IdentityAttackLevel.LOW == "low"

    def test_level_info(self):
        assert IdentityAttackLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = IdentityAttackRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == IdentityAttackType.VULNERABILITY
        assert r.source == IdentityAttackSource.SCANNER
        assert r.level == IdentityAttackLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = IdentityAttackAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == IdentityAttackType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=IdentityAttackType.VULNERABILITY)
        e.record("b", record_type=IdentityAttackType.MISCONFIGURATION)
        results = e.list_records(record_type=IdentityAttackType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=IdentityAttackSource.SCANNER)
        e.record("b", source=IdentityAttackSource.SIEM)
        results = e.list_records(source=IdentityAttackSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = IdentityAttackType.VULNERABILITY
        t2 = IdentityAttackType.MISCONFIGURATION
