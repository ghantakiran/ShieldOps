"""Tests for shieldops.security.credential_compromise_detector — CredentialCompromiseDetector."""

from __future__ import annotations

from shieldops.security.credential_compromise_detector import (
    CredentialAnalysis,
...
def _engine(**kw) -> CredentialCompromiseDetector:
    return CredentialCompromiseDetector(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert CredentialType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert CredentialType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert CredentialType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert CredentialType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert CredentialType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert CredentialSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert CredentialSource.SIEM == "siem"

    def test_source_edr(self):
        assert CredentialSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert CredentialSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert CredentialSource.MANUAL == "manual"

    def test_level_critical(self):
        assert CredentialLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert CredentialLevel.HIGH == "high"

    def test_level_medium(self):
        assert CredentialLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert CredentialLevel.LOW == "low"

    def test_level_info(self):
        assert CredentialLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == CredentialType.VULNERABILITY
        assert r.source == CredentialSource.SCANNER
        assert r.level == CredentialLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = CredentialAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == CredentialType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=CredentialType.VULNERABILITY)
        e.record("b", record_type=CredentialType.MISCONFIGURATION)
        results = e.list_records(record_type=CredentialType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=CredentialSource.SCANNER)
        e.record("b", source=CredentialSource.SIEM)
        results = e.list_records(source=CredentialSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = CredentialType.VULNERABILITY
        t2 = CredentialType.MISCONFIGURATION
