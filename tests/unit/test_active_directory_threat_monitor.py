"""Tests for shieldops.security.active_directory_threat_monitor — ActiveDirectoryThreatMonitor."""

from __future__ import annotations

from shieldops.security.active_directory_threat_monitor import (
    ActiveDirectoryAnalysis,
    ActiveDirectoryLevel,
    ActiveDirectoryRecord,
    ActiveDirectoryReport,
...
def _engine(**kw) -> ActiveDirectoryThreatMonitor:
    return ActiveDirectoryThreatMonitor(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert ActiveDirectoryType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert ActiveDirectoryType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert ActiveDirectoryType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert ActiveDirectoryType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert ActiveDirectoryType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert ActiveDirectorySource.SCANNER == "scanner"

    def test_source_siem(self):
        assert ActiveDirectorySource.SIEM == "siem"

    def test_source_edr(self):
        assert ActiveDirectorySource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert ActiveDirectorySource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert ActiveDirectorySource.MANUAL == "manual"

    def test_level_critical(self):
        assert ActiveDirectoryLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert ActiveDirectoryLevel.HIGH == "high"

    def test_level_medium(self):
        assert ActiveDirectoryLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert ActiveDirectoryLevel.LOW == "low"

    def test_level_info(self):
        assert ActiveDirectoryLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = ActiveDirectoryRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == ActiveDirectoryType.VULNERABILITY
        assert r.source == ActiveDirectorySource.SCANNER
        assert r.level == ActiveDirectoryLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = ActiveDirectoryAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == ActiveDirectoryType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=ActiveDirectoryType.VULNERABILITY)
        e.record("b", record_type=ActiveDirectoryType.MISCONFIGURATION)
        results = e.list_records(record_type=ActiveDirectoryType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=ActiveDirectorySource.SCANNER)
        e.record("b", source=ActiveDirectorySource.SIEM)
        results = e.list_records(source=ActiveDirectorySource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = ActiveDirectoryType.VULNERABILITY
        t2 = ActiveDirectoryType.MISCONFIGURATION
