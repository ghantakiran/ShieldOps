"""Tests for KafkaTelemetryPipeline."""

from __future__ import annotations

from shieldops.observability.kafka_telemetry_pipeline import (
...
def _engine(**kw) -> KafkaTelemetryPipeline:
    return KafkaTelemetryPipeline(**kw)
...
class TestEnums:
    ...
    def test_message_encoding_values(self):
        for v in MessageEncoding:
            assert isinstance(v.value, str)

    def test_pipeline_health_values(self):
        for v in PipelineHealth:
            assert isinstance(v.value, str)
...
class TestComputePipelineThroughput:
    def test_with_data(self):
        ...
        result = eng.compute_pipeline_throughput()
        assert "receiver" in result

    def test_empty(self):
        eng = _engine()
        assert eng.compute_pipeline_throughput() == {}
...
class TestDetectBackpressure:
    def test_with_data(self):
        ...
        eng.add_record(
            name="a",
            pipeline_health=PipelineHealth.BACKPRESSURE,
        )
...
class TestOptimizeBatchConfig:
    def test_with_data(self):
        eng = _engine()
        eng.add_record(name="a", service="svc-a", score=30.0)
        result = eng.optimize_batch_config()
        assert len(result) > 0

    def test_empty(self):
        eng = _engine()
        assert eng.optimize_batch_config() == []
