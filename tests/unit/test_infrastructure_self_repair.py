"""Tests for shieldops.operations.infrastructure_self_repair — InfrastructureSelfRepair."""

from __future__ import annotations

from shieldops.operations.infrastructure_self_repair import (
...
def _engine(**kw) -> InfrastructureSelfRepair:
    return InfrastructureSelfRepair(**kw)


class TestEnums:
    def test_type_restart(self):
        assert InfrastructureType.RESTART == "restart"

    def test_type_scale(self):
        assert InfrastructureType.SCALE == "scale"

    def test_type_patch(self):
        assert InfrastructureType.PATCH == "patch"

    def test_type_rollback(self):
        assert InfrastructureType.ROLLBACK == "rollback"

    def test_type_config_change(self):
        assert InfrastructureType.CONFIG_CHANGE == "config_change"

    def test_source_monitoring(self):
        assert InfrastructureSource.MONITORING == "monitoring"

    def test_source_alert(self):
        assert InfrastructureSource.ALERT == "alert"

    def test_source_schedule(self):
        assert InfrastructureSource.SCHEDULE == "schedule"

    def test_source_manual(self):
        assert InfrastructureSource.MANUAL == "manual"

    def test_source_auto_detect(self):
        assert InfrastructureSource.AUTO_DETECT == "auto_detect"
    ...
    def test_level_routine(self):
        assert InfrastructureLevel.ROUTINE == "routine"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == InfrastructureType.RESTART
        assert r.source == InfrastructureSource.MONITORING
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == InfrastructureType.RESTART
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=InfrastructureType.RESTART)
        e.record("b", record_type=InfrastructureType.SCALE)
        results = e.list_records(record_type=InfrastructureType.RESTART)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=InfrastructureSource.MONITORING)
        e.record("b", source=InfrastructureSource.ALERT)
        results = e.list_records(source=InfrastructureSource.MONITORING)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = InfrastructureType.RESTART
        t2 = InfrastructureType.SCALE
