"""Tests for shieldops.analytics.playbook_performance_analyzer — PlaybookPerformanceAnalyzer."""

from __future__ import annotations

from shieldops.analytics.playbook_performance_analyzer import (
    PlaybookAnalysis,
    PlaybookLevel,
...
def _engine(**kw) -> PlaybookPerformanceAnalyzer:
    return PlaybookPerformanceAnalyzer(**kw)


class TestEnums:
    def test_type_metric(self):
        assert PlaybookType.METRIC == "metric"

    def test_type_trend(self):
        assert PlaybookType.TREND == "trend"

    def test_type_forecast(self):
        assert PlaybookType.FORECAST == "forecast"

    def test_type_anomaly(self):
        assert PlaybookType.ANOMALY == "anomaly"

    def test_type_benchmark(self):
        assert PlaybookType.BENCHMARK == "benchmark"

    def test_source_telemetry(self):
        assert PlaybookSource.TELEMETRY == "telemetry"

    def test_source_log_analysis(self):
        assert PlaybookSource.LOG_ANALYSIS == "log_analysis"

    def test_source_apm(self):
        assert PlaybookSource.APM == "apm"

    def test_source_custom(self):
        assert PlaybookSource.CUSTOM == "custom"

    def test_source_synthetic(self):
        assert PlaybookSource.SYNTHETIC == "synthetic"

    def test_level_critical(self):
        assert PlaybookLevel.CRITICAL == "critical"

    def test_level_warning(self):
        assert PlaybookLevel.WARNING == "warning"

    def test_level_normal(self):
        assert PlaybookLevel.NORMAL == "normal"

    def test_level_optimal(self):
        assert PlaybookLevel.OPTIMAL == "optimal"

    def test_level_unknown(self):
        assert PlaybookLevel.UNKNOWN == "unknown"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == PlaybookType.METRIC
        assert r.source == PlaybookSource.TELEMETRY
        assert r.level == PlaybookLevel.NORMAL
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == PlaybookType.METRIC
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=PlaybookType.METRIC)
        e.record("b", record_type=PlaybookType.TREND)
        results = e.list_records(record_type=PlaybookType.METRIC)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=PlaybookSource.TELEMETRY)
        e.record("b", source=PlaybookSource.LOG_ANALYSIS)
        results = e.list_records(source=PlaybookSource.TELEMETRY)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = PlaybookType.METRIC
        t2 = PlaybookType.TREND
