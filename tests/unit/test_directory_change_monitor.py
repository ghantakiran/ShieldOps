"""Tests for shieldops.security.directory_change_monitor — DirectoryChangeMonitor."""

from __future__ import annotations

from shieldops.security.directory_change_monitor import (
    DirectoryChangeAnalysis,
    DirectoryChangeLevel,
    DirectoryChangeMonitor,
    DirectoryChangeRecord,
    DirectoryChangeReport,
    DirectoryChangeSource,
    DirectoryChangeType,
)


def _engine(**kw) -> DirectoryChangeMonitor:
    return DirectoryChangeMonitor(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert DirectoryChangeType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert DirectoryChangeType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert DirectoryChangeType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert DirectoryChangeType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert DirectoryChangeType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert DirectoryChangeSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert DirectoryChangeSource.SIEM == "siem"

    def test_source_edr(self):
        assert DirectoryChangeSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert DirectoryChangeSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert DirectoryChangeSource.MANUAL == "manual"

    def test_level_critical(self):
        assert DirectoryChangeLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert DirectoryChangeLevel.HIGH == "high"

    def test_level_medium(self):
        assert DirectoryChangeLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert DirectoryChangeLevel.LOW == "low"

    def test_level_info(self):
        assert DirectoryChangeLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = DirectoryChangeRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == DirectoryChangeType.VULNERABILITY
        assert r.source == DirectoryChangeSource.SCANNER
        assert r.level == DirectoryChangeLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = DirectoryChangeAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == DirectoryChangeType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=DirectoryChangeType.VULNERABILITY)
        e.record("b", record_type=DirectoryChangeType.MISCONFIGURATION)
        results = e.list_records(record_type=DirectoryChangeType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=DirectoryChangeSource.SCANNER)
        e.record("b", source=DirectoryChangeSource.SIEM)
        results = e.list_records(source=DirectoryChangeSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = DirectoryChangeType.VULNERABILITY
        t2 = DirectoryChangeType.MISCONFIGURATION
