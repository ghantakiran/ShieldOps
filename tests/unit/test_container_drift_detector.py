"""Tests for shieldops.security.container_drift_detector — ContainerDriftDetector."""

from __future__ import annotations

from shieldops.security.container_drift_detector import (
    ContainerDriftAnalysis,
    ContainerDriftDetector,
    ContainerDriftLevel,
    ContainerDriftRecord,
    ContainerDriftReport,
    ContainerDriftSource,
    ContainerDriftType,
)


def _engine(**kw) -> ContainerDriftDetector:
    return ContainerDriftDetector(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert ContainerDriftType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert ContainerDriftType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert ContainerDriftType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert ContainerDriftType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert ContainerDriftType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert ContainerDriftSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert ContainerDriftSource.SIEM == "siem"

    def test_source_edr(self):
        assert ContainerDriftSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert ContainerDriftSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert ContainerDriftSource.MANUAL == "manual"

    def test_level_critical(self):
        assert ContainerDriftLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert ContainerDriftLevel.HIGH == "high"

    def test_level_medium(self):
        assert ContainerDriftLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert ContainerDriftLevel.LOW == "low"

    def test_level_info(self):
        assert ContainerDriftLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = ContainerDriftRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == ContainerDriftType.VULNERABILITY
        assert r.source == ContainerDriftSource.SCANNER
        assert r.level == ContainerDriftLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = ContainerDriftAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == ContainerDriftType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=ContainerDriftType.VULNERABILITY)
        e.record("b", record_type=ContainerDriftType.MISCONFIGURATION)
        results = e.list_records(record_type=ContainerDriftType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=ContainerDriftSource.SCANNER)
        e.record("b", source=ContainerDriftSource.SIEM)
        results = e.list_records(source=ContainerDriftSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = ContainerDriftType.VULNERABILITY
        t2 = ContainerDriftType.MISCONFIGURATION
