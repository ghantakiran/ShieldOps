"""Tests for shieldops.security.automated_triage_engine — AutomatedTriageEngine."""

from __future__ import annotations

from shieldops.security.automated_triage_engine import (
    AutomatedTriageAnalysis,
    AutomatedTriageEngine,
    AutomatedTriageLevel,
    AutomatedTriageRecord,
    AutomatedTriageReport,
    AutomatedTriageSource,
    AutomatedTriageType,
)


def _engine(**kw) -> AutomatedTriageEngine:
    return AutomatedTriageEngine(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert AutomatedTriageType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert AutomatedTriageType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert AutomatedTriageType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert AutomatedTriageType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert AutomatedTriageType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert AutomatedTriageSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert AutomatedTriageSource.SIEM == "siem"

    def test_source_edr(self):
        assert AutomatedTriageSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert AutomatedTriageSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert AutomatedTriageSource.MANUAL == "manual"

    def test_level_critical(self):
        assert AutomatedTriageLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert AutomatedTriageLevel.HIGH == "high"

    def test_level_medium(self):
        assert AutomatedTriageLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert AutomatedTriageLevel.LOW == "low"

    def test_level_info(self):
        assert AutomatedTriageLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = AutomatedTriageRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == AutomatedTriageType.VULNERABILITY
        assert r.source == AutomatedTriageSource.SCANNER
        assert r.level == AutomatedTriageLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = AutomatedTriageAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == AutomatedTriageType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=AutomatedTriageType.VULNERABILITY)
        e.record("b", record_type=AutomatedTriageType.MISCONFIGURATION)
        results = e.list_records(record_type=AutomatedTriageType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=AutomatedTriageSource.SCANNER)
        e.record("b", source=AutomatedTriageSource.SIEM)
        results = e.list_records(source=AutomatedTriageSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = AutomatedTriageType.VULNERABILITY
        t2 = AutomatedTriageType.MISCONFIGURATION
