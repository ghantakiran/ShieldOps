"""Tests for shieldops.security.indicator_enrichment_pipeline — IndicatorEnrichmentPipeline."""

from __future__ import annotations

from shieldops.security.indicator_enrichment_pipeline import (
    IndicatorAnalysis,
    IndicatorEnrichmentPipeline,
    IndicatorLevel,
    IndicatorRecord,
    IndicatorReport,
    IndicatorSource,
...
def _engine(**kw) -> IndicatorEnrichmentPipeline:
    return IndicatorEnrichmentPipeline(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert IndicatorType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert IndicatorType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert IndicatorType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert IndicatorType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert IndicatorType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert IndicatorSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert IndicatorSource.SIEM == "siem"

    def test_source_edr(self):
        assert IndicatorSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert IndicatorSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert IndicatorSource.MANUAL == "manual"

    def test_level_critical(self):
        assert IndicatorLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert IndicatorLevel.HIGH == "high"

    def test_level_medium(self):
        assert IndicatorLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert IndicatorLevel.LOW == "low"

    def test_level_info(self):
        assert IndicatorLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == IndicatorType.VULNERABILITY
        assert r.source == IndicatorSource.SCANNER
        assert r.level == IndicatorLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == IndicatorType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=IndicatorType.VULNERABILITY)
        e.record("b", record_type=IndicatorType.MISCONFIGURATION)
        results = e.list_records(record_type=IndicatorType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=IndicatorSource.SCANNER)
        e.record("b", source=IndicatorSource.SIEM)
        results = e.list_records(source=IndicatorSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = IndicatorType.VULNERABILITY
        t2 = IndicatorType.MISCONFIGURATION
