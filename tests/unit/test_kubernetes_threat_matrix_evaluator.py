"""
Tests for shieldops.security.kubernetes_threat_matrix_evaluator — KubernetesThreatMatrixEvaluator.
"""

from __future__ import annotations

from shieldops.security.kubernetes_threat_matrix_evaluator import (
    KubernetesThreatAnalysis,
    KubernetesThreatLevel,
    KubernetesThreatMatrixEvaluator,
    KubernetesThreatRecord,
    KubernetesThreatReport,
    KubernetesThreatSource,
    KubernetesThreatType,
)


def _engine(**kw) -> KubernetesThreatMatrixEvaluator:
    return KubernetesThreatMatrixEvaluator(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert KubernetesThreatType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert KubernetesThreatType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert KubernetesThreatType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert KubernetesThreatType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert KubernetesThreatType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert KubernetesThreatSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert KubernetesThreatSource.SIEM == "siem"

    def test_source_edr(self):
        assert KubernetesThreatSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert KubernetesThreatSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert KubernetesThreatSource.MANUAL == "manual"

    def test_level_critical(self):
        assert KubernetesThreatLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert KubernetesThreatLevel.HIGH == "high"

    def test_level_medium(self):
        assert KubernetesThreatLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert KubernetesThreatLevel.LOW == "low"

    def test_level_info(self):
        assert KubernetesThreatLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = KubernetesThreatRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == KubernetesThreatType.VULNERABILITY
        assert r.source == KubernetesThreatSource.SCANNER
        assert r.level == KubernetesThreatLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = KubernetesThreatAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == KubernetesThreatType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=KubernetesThreatType.VULNERABILITY)
        e.record("b", record_type=KubernetesThreatType.MISCONFIGURATION)
        results = e.list_records(record_type=KubernetesThreatType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=KubernetesThreatSource.SCANNER)
        e.record("b", source=KubernetesThreatSource.SIEM)
        results = e.list_records(source=KubernetesThreatSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = KubernetesThreatType.VULNERABILITY
        t2 = KubernetesThreatType.MISCONFIGURATION
