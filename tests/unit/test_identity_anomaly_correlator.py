"""Tests for shieldops.security.identity_anomaly_correlator — IdentityAnomalyCorrelator."""

from __future__ import annotations

from shieldops.security.identity_anomaly_correlator import (
    IdentityAnomalyAnalysis,
    IdentityAnomalyCorrelator,
    IdentityAnomalyLevel,
    IdentityAnomalyRecord,
    IdentityAnomalyReport,
    IdentityAnomalySource,
    IdentityAnomalyType,
)


def _engine(**kw) -> IdentityAnomalyCorrelator:
    return IdentityAnomalyCorrelator(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert IdentityAnomalyType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert IdentityAnomalyType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert IdentityAnomalyType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert IdentityAnomalyType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert IdentityAnomalyType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert IdentityAnomalySource.SCANNER == "scanner"

    def test_source_siem(self):
        assert IdentityAnomalySource.SIEM == "siem"

    def test_source_edr(self):
        assert IdentityAnomalySource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert IdentityAnomalySource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert IdentityAnomalySource.MANUAL == "manual"

    def test_level_critical(self):
        assert IdentityAnomalyLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert IdentityAnomalyLevel.HIGH == "high"

    def test_level_medium(self):
        assert IdentityAnomalyLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert IdentityAnomalyLevel.LOW == "low"

    def test_level_info(self):
        assert IdentityAnomalyLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = IdentityAnomalyRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == IdentityAnomalyType.VULNERABILITY
        assert r.source == IdentityAnomalySource.SCANNER
        assert r.level == IdentityAnomalyLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = IdentityAnomalyAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == IdentityAnomalyType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=IdentityAnomalyType.VULNERABILITY)
        e.record("b", record_type=IdentityAnomalyType.MISCONFIGURATION)
        results = e.list_records(record_type=IdentityAnomalyType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=IdentityAnomalySource.SCANNER)
        e.record("b", source=IdentityAnomalySource.SIEM)
        results = e.list_records(source=IdentityAnomalySource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = IdentityAnomalyType.VULNERABILITY
        t2 = IdentityAnomalyType.MISCONFIGURATION
