"""Tests for shieldops.operations.canary_deployment_automator — CanaryDeploymentAutomator."""

from __future__ import annotations

from shieldops.operations.canary_deployment_automator import (
    CanaryDeploymentAnalysis,
    CanaryDeploymentAutomator,
    CanaryDeploymentLevel,
    CanaryDeploymentRecord,
    CanaryDeploymentReport,
    CanaryDeploymentSource,
    CanaryDeploymentType,
)


def _engine(**kw) -> CanaryDeploymentAutomator:
    return CanaryDeploymentAutomator(**kw)


class TestEnums:
    def test_type_restart(self):
        assert CanaryDeploymentType.RESTART == "restart"

    def test_type_scale(self):
        assert CanaryDeploymentType.SCALE == "scale"

    def test_type_patch(self):
        assert CanaryDeploymentType.PATCH == "patch"

    def test_type_rollback(self):
        assert CanaryDeploymentType.ROLLBACK == "rollback"

    def test_type_config_change(self):
        assert CanaryDeploymentType.CONFIG_CHANGE == "config_change"

    def test_source_monitoring(self):
        assert CanaryDeploymentSource.MONITORING == "monitoring"

    def test_source_alert(self):
        assert CanaryDeploymentSource.ALERT == "alert"

    def test_source_schedule(self):
        assert CanaryDeploymentSource.SCHEDULE == "schedule"

    def test_source_manual(self):
        assert CanaryDeploymentSource.MANUAL == "manual"

    def test_source_auto_detect(self):
        assert CanaryDeploymentSource.AUTO_DETECT == "auto_detect"

    def test_level_critical(self):
        assert CanaryDeploymentLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert CanaryDeploymentLevel.HIGH == "high"

    def test_level_medium(self):
        assert CanaryDeploymentLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert CanaryDeploymentLevel.LOW == "low"

    def test_level_routine(self):
        assert CanaryDeploymentLevel.ROUTINE == "routine"


class TestModels:
    def test_record_defaults(self):
        r = CanaryDeploymentRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == CanaryDeploymentType.RESTART
        assert r.source == CanaryDeploymentSource.MONITORING
        assert r.level == CanaryDeploymentLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = CanaryDeploymentAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == CanaryDeploymentType.RESTART
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=CanaryDeploymentType.RESTART)
        e.record("b", record_type=CanaryDeploymentType.SCALE)
        results = e.list_records(record_type=CanaryDeploymentType.RESTART)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=CanaryDeploymentSource.MONITORING)
        e.record("b", source=CanaryDeploymentSource.ALERT)
        results = e.list_records(source=CanaryDeploymentSource.MONITORING)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = CanaryDeploymentType.RESTART
        t2 = CanaryDeploymentType.SCALE
