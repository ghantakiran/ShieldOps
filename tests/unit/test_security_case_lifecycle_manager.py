"""Tests for shieldops.security.security_case_lifecycle_manager — SecurityCaseLifecycleManager."""

from __future__ import annotations

from shieldops.security.security_case_lifecycle_manager import (
    SecurityCaseAnalysis,
    SecurityCaseLevel,
    SecurityCaseLifecycleManager,
    SecurityCaseRecord,
    SecurityCaseReport,
    SecurityCaseSource,
    SecurityCaseType,
...
def _engine(**kw) -> SecurityCaseLifecycleManager:
    return SecurityCaseLifecycleManager(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert SecurityCaseType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert SecurityCaseType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert SecurityCaseType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert SecurityCaseType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert SecurityCaseType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert SecurityCaseSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert SecurityCaseSource.SIEM == "siem"

    def test_source_edr(self):
        assert SecurityCaseSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert SecurityCaseSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert SecurityCaseSource.MANUAL == "manual"

    def test_level_critical(self):
        assert SecurityCaseLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert SecurityCaseLevel.HIGH == "high"

    def test_level_medium(self):
        assert SecurityCaseLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert SecurityCaseLevel.LOW == "low"

    def test_level_info(self):
        assert SecurityCaseLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = SecurityCaseRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == SecurityCaseType.VULNERABILITY
        assert r.source == SecurityCaseSource.SCANNER
        assert r.level == SecurityCaseLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = SecurityCaseAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == SecurityCaseType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=SecurityCaseType.VULNERABILITY)
        e.record("b", record_type=SecurityCaseType.MISCONFIGURATION)
        results = e.list_records(record_type=SecurityCaseType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=SecurityCaseSource.SCANNER)
        e.record("b", source=SecurityCaseSource.SIEM)
        results = e.list_records(source=SecurityCaseSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = SecurityCaseType.VULNERABILITY
        t2 = SecurityCaseType.MISCONFIGURATION
