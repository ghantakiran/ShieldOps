"""Tests for shieldops.security.cloud_network_micro_segmentation — CloudNetworkMicroSegmentation."""

from __future__ import annotations

from shieldops.security.cloud_network_micro_segmentation import (
    CloudNetworkAnalysis,
    CloudNetworkLevel,
    CloudNetworkMicroSegmentation,
    CloudNetworkRecord,
    CloudNetworkReport,
    CloudNetworkSource,
    CloudNetworkType,
...
def _engine(**kw) -> CloudNetworkMicroSegmentation:
    return CloudNetworkMicroSegmentation(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert CloudNetworkType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert CloudNetworkType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert CloudNetworkType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert CloudNetworkType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert CloudNetworkType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert CloudNetworkSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert CloudNetworkSource.SIEM == "siem"

    def test_source_edr(self):
        assert CloudNetworkSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert CloudNetworkSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert CloudNetworkSource.MANUAL == "manual"

    def test_level_critical(self):
        assert CloudNetworkLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert CloudNetworkLevel.HIGH == "high"

    def test_level_medium(self):
        assert CloudNetworkLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert CloudNetworkLevel.LOW == "low"

    def test_level_info(self):
        assert CloudNetworkLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = CloudNetworkRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == CloudNetworkType.VULNERABILITY
        assert r.source == CloudNetworkSource.SCANNER
        assert r.level == CloudNetworkLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = CloudNetworkAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == CloudNetworkType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=CloudNetworkType.VULNERABILITY)
        e.record("b", record_type=CloudNetworkType.MISCONFIGURATION)
        results = e.list_records(record_type=CloudNetworkType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=CloudNetworkSource.SCANNER)
        e.record("b", source=CloudNetworkSource.SIEM)
        results = e.list_records(source=CloudNetworkSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = CloudNetworkType.VULNERABILITY
        t2 = CloudNetworkType.MISCONFIGURATION
