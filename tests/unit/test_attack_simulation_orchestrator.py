"""Tests for shieldops.security.attack_simulation_orchestrator — AttackSimulationOrchestrator."""

from __future__ import annotations

from shieldops.security.attack_simulation_orchestrator import (
    AttackSimulationAnalysis,
    AttackSimulationLevel,
    AttackSimulationOrchestrator,
    AttackSimulationRecord,
    AttackSimulationReport,
    AttackSimulationSource,
    AttackSimulationType,
)


def _engine(**kw) -> AttackSimulationOrchestrator:
    return AttackSimulationOrchestrator(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert AttackSimulationType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert AttackSimulationType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert AttackSimulationType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert AttackSimulationType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert AttackSimulationType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert AttackSimulationSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert AttackSimulationSource.SIEM == "siem"

    def test_source_edr(self):
        assert AttackSimulationSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert AttackSimulationSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert AttackSimulationSource.MANUAL == "manual"

    def test_level_critical(self):
        assert AttackSimulationLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert AttackSimulationLevel.HIGH == "high"

    def test_level_medium(self):
        assert AttackSimulationLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert AttackSimulationLevel.LOW == "low"

    def test_level_info(self):
        assert AttackSimulationLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = AttackSimulationRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == AttackSimulationType.VULNERABILITY
        assert r.source == AttackSimulationSource.SCANNER
        assert r.level == AttackSimulationLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = AttackSimulationAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == AttackSimulationType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=AttackSimulationType.VULNERABILITY)
        e.record("b", record_type=AttackSimulationType.MISCONFIGURATION)
        results = e.list_records(record_type=AttackSimulationType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=AttackSimulationSource.SCANNER)
        e.record("b", source=AttackSimulationSource.SIEM)
        results = e.list_records(source=AttackSimulationSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = AttackSimulationType.VULNERABILITY
        t2 = AttackSimulationType.MISCONFIGURATION
