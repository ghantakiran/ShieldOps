"""Tests for shieldops.security.cyber_risk_quantifier — CyberRiskQuantifier."""

from __future__ import annotations

from shieldops.security.cyber_risk_quantifier import (
    CyberRiskAnalysis,
    CyberRiskLevel,
    CyberRiskQuantifier,
    CyberRiskRecord,
...
def _engine(**kw) -> CyberRiskQuantifier:
    return CyberRiskQuantifier(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert CyberRiskType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert CyberRiskType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert CyberRiskType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert CyberRiskType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert CyberRiskType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert CyberRiskSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert CyberRiskSource.SIEM == "siem"

    def test_source_edr(self):
        assert CyberRiskSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert CyberRiskSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert CyberRiskSource.MANUAL == "manual"

    def test_level_critical(self):
        assert CyberRiskLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert CyberRiskLevel.HIGH == "high"

    def test_level_medium(self):
        assert CyberRiskLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert CyberRiskLevel.LOW == "low"

    def test_level_info(self):
        assert CyberRiskLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == CyberRiskType.VULNERABILITY
        assert r.source == CyberRiskSource.SCANNER
        assert r.level == CyberRiskLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == CyberRiskType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=CyberRiskType.VULNERABILITY)
        e.record("b", record_type=CyberRiskType.MISCONFIGURATION)
        results = e.list_records(record_type=CyberRiskType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=CyberRiskSource.SCANNER)
        e.record("b", source=CyberRiskSource.SIEM)
        results = e.list_records(source=CyberRiskSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = CyberRiskType.VULNERABILITY
        t2 = CyberRiskType.MISCONFIGURATION
