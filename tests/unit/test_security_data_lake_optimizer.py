"""Tests for shieldops.analytics.security_data_lake_optimizer — SecurityDataLakeOptimizer."""

from __future__ import annotations

from shieldops.analytics.security_data_lake_optimizer import (
    SecurityDataLakeAnalysis,
    SecurityDataLakeLevel,
    SecurityDataLakeOptimizer,
    SecurityDataLakeRecord,
    SecurityDataLakeReport,
    SecurityDataLakeSource,
    SecurityDataLakeType,
)


def _engine(**kw) -> SecurityDataLakeOptimizer:
    return SecurityDataLakeOptimizer(**kw)


class TestEnums:
    def test_type_metric(self):
        assert SecurityDataLakeType.METRIC == "metric"

    def test_type_trend(self):
        assert SecurityDataLakeType.TREND == "trend"

    def test_type_forecast(self):
        assert SecurityDataLakeType.FORECAST == "forecast"

    def test_type_anomaly(self):
        assert SecurityDataLakeType.ANOMALY == "anomaly"

    def test_type_benchmark(self):
        assert SecurityDataLakeType.BENCHMARK == "benchmark"

    def test_source_telemetry(self):
        assert SecurityDataLakeSource.TELEMETRY == "telemetry"

    def test_source_log_analysis(self):
        assert SecurityDataLakeSource.LOG_ANALYSIS == "log_analysis"

    def test_source_apm(self):
        assert SecurityDataLakeSource.APM == "apm"

    def test_source_custom(self):
        assert SecurityDataLakeSource.CUSTOM == "custom"

    def test_source_synthetic(self):
        assert SecurityDataLakeSource.SYNTHETIC == "synthetic"

    def test_level_critical(self):
        assert SecurityDataLakeLevel.CRITICAL == "critical"

    def test_level_warning(self):
        assert SecurityDataLakeLevel.WARNING == "warning"

    def test_level_normal(self):
        assert SecurityDataLakeLevel.NORMAL == "normal"

    def test_level_optimal(self):
        assert SecurityDataLakeLevel.OPTIMAL == "optimal"

    def test_level_unknown(self):
        assert SecurityDataLakeLevel.UNKNOWN == "unknown"


class TestModels:
    def test_record_defaults(self):
        r = SecurityDataLakeRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == SecurityDataLakeType.METRIC
        assert r.source == SecurityDataLakeSource.TELEMETRY
        assert r.level == SecurityDataLakeLevel.NORMAL
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = SecurityDataLakeAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == SecurityDataLakeType.METRIC
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=SecurityDataLakeType.METRIC)
        e.record("b", record_type=SecurityDataLakeType.TREND)
        results = e.list_records(record_type=SecurityDataLakeType.METRIC)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=SecurityDataLakeSource.TELEMETRY)
        e.record("b", source=SecurityDataLakeSource.LOG_ANALYSIS)
        results = e.list_records(source=SecurityDataLakeSource.TELEMETRY)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_single_type(self):
        ...
        assert dist["metric"]["count"] == 2
        assert dist["metric"]["avg_score"] == 70.0

    def test_multiple_types(self):
        e = _engine()
        t1 = SecurityDataLakeType.METRIC
        t2 = SecurityDataLakeType.TREND
