"""Tests for shieldops.security.service_account_risk_analyzer — ServiceAccountRiskAnalyzer."""

from __future__ import annotations

from shieldops.security.service_account_risk_analyzer import (
    ServiceAccountAnalysis,
    ServiceAccountLevel,
    ServiceAccountRecord,
    ServiceAccountReport,
    ServiceAccountRiskAnalyzer,
    ServiceAccountSource,
    ServiceAccountType,
)


def _engine(**kw) -> ServiceAccountRiskAnalyzer:
    return ServiceAccountRiskAnalyzer(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert ServiceAccountType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert ServiceAccountType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert ServiceAccountType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert ServiceAccountType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert ServiceAccountType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert ServiceAccountSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert ServiceAccountSource.SIEM == "siem"

    def test_source_edr(self):
        assert ServiceAccountSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert ServiceAccountSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert ServiceAccountSource.MANUAL == "manual"

    def test_level_critical(self):
        assert ServiceAccountLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert ServiceAccountLevel.HIGH == "high"

    def test_level_medium(self):
        assert ServiceAccountLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert ServiceAccountLevel.LOW == "low"

    def test_level_info(self):
        assert ServiceAccountLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == ServiceAccountType.VULNERABILITY
        assert r.source == ServiceAccountSource.SCANNER
        assert r.level == ServiceAccountLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = ServiceAccountAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == ServiceAccountType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=ServiceAccountType.VULNERABILITY)
        e.record("b", record_type=ServiceAccountType.MISCONFIGURATION)
        results = e.list_records(record_type=ServiceAccountType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=ServiceAccountSource.SCANNER)
        e.record("b", source=ServiceAccountSource.SIEM)
        results = e.list_records(source=ServiceAccountSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = ServiceAccountType.VULNERABILITY
        t2 = ServiceAccountType.MISCONFIGURATION
