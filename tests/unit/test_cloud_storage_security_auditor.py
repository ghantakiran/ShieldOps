"""Tests for shieldops.security.cloud_storage_security_auditor — CloudStorageSecurityAuditor."""

from __future__ import annotations

from shieldops.security.cloud_storage_security_auditor import (
    CloudStorageAnalysis,
    CloudStorageLevel,
    CloudStorageRecord,
    CloudStorageReport,
    CloudStorageSecurityAuditor,
    CloudStorageSource,
    CloudStorageType,
)


def _engine(**kw) -> CloudStorageSecurityAuditor:
    return CloudStorageSecurityAuditor(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert CloudStorageType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert CloudStorageType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert CloudStorageType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert CloudStorageType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert CloudStorageType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert CloudStorageSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert CloudStorageSource.SIEM == "siem"

    def test_source_edr(self):
        assert CloudStorageSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert CloudStorageSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert CloudStorageSource.MANUAL == "manual"

    def test_level_critical(self):
        assert CloudStorageLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert CloudStorageLevel.HIGH == "high"

    def test_level_medium(self):
        assert CloudStorageLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert CloudStorageLevel.LOW == "low"

    def test_level_info(self):
        assert CloudStorageLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = CloudStorageRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == CloudStorageType.VULNERABILITY
        assert r.source == CloudStorageSource.SCANNER
        assert r.level == CloudStorageLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = CloudStorageAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == CloudStorageType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=CloudStorageType.VULNERABILITY)
        e.record("b", record_type=CloudStorageType.MISCONFIGURATION)
        results = e.list_records(record_type=CloudStorageType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=CloudStorageSource.SCANNER)
        e.record("b", source=CloudStorageSource.SIEM)
        results = e.list_records(source=CloudStorageSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = CloudStorageType.VULNERABILITY
        t2 = CloudStorageType.MISCONFIGURATION
