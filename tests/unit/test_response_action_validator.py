"""Tests for shieldops.security.response_action_validator — ResponseActionValidator."""

from __future__ import annotations

from shieldops.security.response_action_validator import (
    ResponseActionAnalysis,
    ResponseActionLevel,
    ResponseActionRecord,
    ResponseActionReport,
...
def _engine(**kw) -> ResponseActionValidator:
    return ResponseActionValidator(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert ResponseActionType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert ResponseActionType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert ResponseActionType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert ResponseActionType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert ResponseActionType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert ResponseActionSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert ResponseActionSource.SIEM == "siem"

    def test_source_edr(self):
        assert ResponseActionSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert ResponseActionSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert ResponseActionSource.MANUAL == "manual"

    def test_level_critical(self):
        assert ResponseActionLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert ResponseActionLevel.HIGH == "high"

    def test_level_medium(self):
        assert ResponseActionLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert ResponseActionLevel.LOW == "low"

    def test_level_info(self):
        assert ResponseActionLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = ResponseActionRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == ResponseActionType.VULNERABILITY
        assert r.source == ResponseActionSource.SCANNER
        assert r.level == ResponseActionLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = ResponseActionAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == ResponseActionType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=ResponseActionType.VULNERABILITY)
        e.record("b", record_type=ResponseActionType.MISCONFIGURATION)
        results = e.list_records(record_type=ResponseActionType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=ResponseActionSource.SCANNER)
        e.record("b", source=ResponseActionSource.SIEM)
        results = e.list_records(source=ResponseActionSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = ResponseActionType.VULNERABILITY
        t2 = ResponseActionType.MISCONFIGURATION
