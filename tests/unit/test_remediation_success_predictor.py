"""Tests for shieldops.analytics.remediation_success_predictor — RemediationSuccessPredictor."""

from __future__ import annotations

from shieldops.analytics.remediation_success_predictor import (
    RemediationAnalysis,
    RemediationLevel,
    RemediationRecord,
    RemediationReport,
...
def _engine(**kw) -> RemediationSuccessPredictor:
    return RemediationSuccessPredictor(**kw)


class TestEnums:
    def test_type_metric(self):
        assert RemediationType.METRIC == "metric"

    def test_type_trend(self):
        assert RemediationType.TREND == "trend"

    def test_type_forecast(self):
        assert RemediationType.FORECAST == "forecast"

    def test_type_anomaly(self):
        assert RemediationType.ANOMALY == "anomaly"

    def test_type_benchmark(self):
        assert RemediationType.BENCHMARK == "benchmark"

    def test_source_telemetry(self):
        assert RemediationSource.TELEMETRY == "telemetry"

    def test_source_log_analysis(self):
        assert RemediationSource.LOG_ANALYSIS == "log_analysis"

    def test_source_apm(self):
        assert RemediationSource.APM == "apm"

    def test_source_custom(self):
        assert RemediationSource.CUSTOM == "custom"

    def test_source_synthetic(self):
        assert RemediationSource.SYNTHETIC == "synthetic"

    def test_level_critical(self):
        assert RemediationLevel.CRITICAL == "critical"

    def test_level_warning(self):
        assert RemediationLevel.WARNING == "warning"

    def test_level_normal(self):
        assert RemediationLevel.NORMAL == "normal"

    def test_level_optimal(self):
        assert RemediationLevel.OPTIMAL == "optimal"

    def test_level_unknown(self):
        assert RemediationLevel.UNKNOWN == "unknown"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == RemediationType.METRIC
        assert r.source == RemediationSource.TELEMETRY
        assert r.level == RemediationLevel.NORMAL
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = RemediationAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == RemediationType.METRIC
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=RemediationType.METRIC)
        e.record("b", record_type=RemediationType.TREND)
        results = e.list_records(record_type=RemediationType.METRIC)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=RemediationSource.TELEMETRY)
        e.record("b", source=RemediationSource.LOG_ANALYSIS)
        results = e.list_records(source=RemediationSource.TELEMETRY)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = RemediationType.METRIC
        t2 = RemediationType.TREND
