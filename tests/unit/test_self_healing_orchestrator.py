"""Tests for shieldops.operations.self_healing_orchestrator — SelfHealingOrchestrator."""

from __future__ import annotations

from shieldops.operations.self_healing_orchestrator import (
    SelfHealingAnalysis,
    SelfHealingLevel,
...
class TestEnums:
    def test_type_restart(self):
        assert SelfHealingType.RESTART == "restart"

    def test_type_scale(self):
        assert SelfHealingType.SCALE == "scale"

    def test_type_patch(self):
        assert SelfHealingType.PATCH == "patch"

    def test_type_rollback(self):
        assert SelfHealingType.ROLLBACK == "rollback"

    def test_type_config_change(self):
        assert SelfHealingType.CONFIG_CHANGE == "config_change"

    def test_source_monitoring(self):
        assert SelfHealingSource.MONITORING == "monitoring"

    def test_source_alert(self):
        assert SelfHealingSource.ALERT == "alert"

    def test_source_schedule(self):
        assert SelfHealingSource.SCHEDULE == "schedule"

    def test_source_manual(self):
        assert SelfHealingSource.MANUAL == "manual"

    def test_source_auto_detect(self):
        assert SelfHealingSource.AUTO_DETECT == "auto_detect"

    def test_level_critical(self):
        assert SelfHealingLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert SelfHealingLevel.HIGH == "high"

    def test_level_medium(self):
        assert SelfHealingLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert SelfHealingLevel.LOW == "low"

    def test_level_routine(self):
        assert SelfHealingLevel.ROUTINE == "routine"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == SelfHealingType.RESTART
        assert r.source == SelfHealingSource.MONITORING
        assert r.level == SelfHealingLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == SelfHealingType.RESTART
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=SelfHealingType.RESTART)
        e.record("b", record_type=SelfHealingType.SCALE)
        results = e.list_records(record_type=SelfHealingType.RESTART)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=SelfHealingSource.MONITORING)
        e.record("b", source=SelfHealingSource.ALERT)
        results = e.list_records(source=SelfHealingSource.MONITORING)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = SelfHealingType.RESTART
        t2 = SelfHealingType.SCALE
