"""Tests for shieldops.security.adversary_tactics_profiler — AdversaryTacticsProfiler."""

from __future__ import annotations

from shieldops.security.adversary_tactics_profiler import (
    AdversaryTacticsAnalysis,
    AdversaryTacticsLevel,
    AdversaryTacticsProfiler,
    AdversaryTacticsRecord,
    AdversaryTacticsReport,
    AdversaryTacticsSource,
    AdversaryTacticsType,
)


def _engine(**kw) -> AdversaryTacticsProfiler:
    return AdversaryTacticsProfiler(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert AdversaryTacticsType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert AdversaryTacticsType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert AdversaryTacticsType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert AdversaryTacticsType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert AdversaryTacticsType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert AdversaryTacticsSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert AdversaryTacticsSource.SIEM == "siem"

    def test_source_edr(self):
        assert AdversaryTacticsSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert AdversaryTacticsSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert AdversaryTacticsSource.MANUAL == "manual"

    def test_level_critical(self):
        assert AdversaryTacticsLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert AdversaryTacticsLevel.HIGH == "high"

    def test_level_medium(self):
        assert AdversaryTacticsLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert AdversaryTacticsLevel.LOW == "low"

    def test_level_info(self):
        assert AdversaryTacticsLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = AdversaryTacticsRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == AdversaryTacticsType.VULNERABILITY
        assert r.source == AdversaryTacticsSource.SCANNER
        assert r.level == AdversaryTacticsLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = AdversaryTacticsAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == AdversaryTacticsType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=AdversaryTacticsType.VULNERABILITY)
        e.record("b", record_type=AdversaryTacticsType.MISCONFIGURATION)
        results = e.list_records(record_type=AdversaryTacticsType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=AdversaryTacticsSource.SCANNER)
        e.record("b", source=AdversaryTacticsSource.SIEM)
        results = e.list_records(source=AdversaryTacticsSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = AdversaryTacticsType.VULNERABILITY
        t2 = AdversaryTacticsType.MISCONFIGURATION
