"""Tests for InfrastructureEntitlementAnalyzer."""

from __future__ import annotations

from shieldops.security.infrastructure_entitlement_analyzer import (
    InfrastructureAnalysis,
    InfrastructureEntitlementAnalyzer,
    InfrastructureLevel,
...
def _engine(**kw) -> InfrastructureEntitlementAnalyzer:
    return InfrastructureEntitlementAnalyzer(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert InfrastructureType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert InfrastructureType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert InfrastructureType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert InfrastructureType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert InfrastructureType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert InfrastructureSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert InfrastructureSource.SIEM == "siem"

    def test_source_edr(self):
        assert InfrastructureSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert InfrastructureSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert InfrastructureSource.MANUAL == "manual"

    def test_level_critical(self):
        assert InfrastructureLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert InfrastructureLevel.HIGH == "high"

    def test_level_medium(self):
        assert InfrastructureLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert InfrastructureLevel.LOW == "low"

    def test_level_info(self):
        assert InfrastructureLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == InfrastructureType.VULNERABILITY
        assert r.source == InfrastructureSource.SCANNER
        assert r.level == InfrastructureLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = InfrastructureAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == InfrastructureType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=InfrastructureType.VULNERABILITY)
        e.record("b", record_type=InfrastructureType.MISCONFIGURATION)
        results = e.list_records(record_type=InfrastructureType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=InfrastructureSource.SCANNER)
        e.record("b", source=InfrastructureSource.SIEM)
        results = e.list_records(source=InfrastructureSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = InfrastructureType.VULNERABILITY
        t2 = InfrastructureType.MISCONFIGURATION
