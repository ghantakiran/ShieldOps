"""Tests for shieldops.compliance.regulatory_automation_tracker — RegulatoryAutomationTracker."""

from __future__ import annotations

from shieldops.compliance.regulatory_automation_tracker import (
    RegulatoryAnalysis,
...
def _engine(**kw) -> RegulatoryAutomationTracker:
    return RegulatoryAutomationTracker(**kw)


class TestEnums:
    def test_type_control(self):
        assert RegulatoryType.CONTROL == "control"

    def test_type_policy(self):
        assert RegulatoryType.POLICY == "policy"

    def test_type_regulation(self):
        assert RegulatoryType.REGULATION == "regulation"

    def test_type_standard(self):
        assert RegulatoryType.STANDARD == "standard"

    def test_type_framework(self):
        assert RegulatoryType.FRAMEWORK == "framework"

    def test_source_audit(self):
        assert RegulatorySource.AUDIT == "audit"

    def test_source_automated_scan(self):
        assert RegulatorySource.AUTOMATED_SCAN == "automated_scan"

    def test_source_manual_review(self):
        assert RegulatorySource.MANUAL_REVIEW == "manual_review"

    def test_source_continuous_monitor(self):
        assert RegulatorySource.CONTINUOUS_MONITOR == "continuous_monitor"

    def test_source_third_party(self):
        assert RegulatorySource.THIRD_PARTY == "third_party"

    def test_level_compliant(self):
        assert RegulatoryLevel.COMPLIANT == "compliant"

    def test_level_partial(self):
        assert RegulatoryLevel.PARTIAL == "partial"

    def test_level_non_compliant(self):
        assert RegulatoryLevel.NON_COMPLIANT == "non_compliant"

    def test_level_not_assessed(self):
        assert RegulatoryLevel.NOT_ASSESSED == "not_assessed"

    def test_level_exempt(self):
        assert RegulatoryLevel.EXEMPT == "exempt"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == RegulatoryType.CONTROL
        assert r.source == RegulatorySource.AUDIT
        assert r.level == RegulatoryLevel.NON_COMPLIANT
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == RegulatoryType.CONTROL
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=RegulatoryType.CONTROL)
        e.record("b", record_type=RegulatoryType.POLICY)
        results = e.list_records(record_type=RegulatoryType.CONTROL)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=RegulatorySource.AUDIT)
        e.record("b", source=RegulatorySource.AUTOMATED_SCAN)
        results = e.list_records(source=RegulatorySource.AUDIT)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = RegulatoryType.CONTROL
        t2 = RegulatoryType.POLICY
