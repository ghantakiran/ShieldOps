"""Tests for shieldops.security.identity_governance_enforcer — IdentityGovernanceEnforcer."""

from __future__ import annotations

from shieldops.security.identity_governance_enforcer import (
    IdentityAnalysis,
    IdentityGovernanceEnforcer,
    IdentityLevel,
    IdentityRecord,
    IdentityReport,
    IdentitySource,
...
def _engine(**kw) -> IdentityGovernanceEnforcer:
    return IdentityGovernanceEnforcer(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert IdentityType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert IdentityType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert IdentityType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert IdentityType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert IdentityType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert IdentitySource.SCANNER == "scanner"

    def test_source_siem(self):
        assert IdentitySource.SIEM == "siem"

    def test_source_edr(self):
        assert IdentitySource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert IdentitySource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert IdentitySource.MANUAL == "manual"

    def test_level_critical(self):
        assert IdentityLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert IdentityLevel.HIGH == "high"

    def test_level_medium(self):
        assert IdentityLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert IdentityLevel.LOW == "low"

    def test_level_info(self):
        assert IdentityLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == IdentityType.VULNERABILITY
        assert r.source == IdentitySource.SCANNER
        assert r.level == IdentityLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == IdentityType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=IdentityType.VULNERABILITY)
        e.record("b", record_type=IdentityType.MISCONFIGURATION)
        results = e.list_records(record_type=IdentityType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=IdentitySource.SCANNER)
        e.record("b", source=IdentitySource.SIEM)
        results = e.list_records(source=IdentitySource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = IdentityType.VULNERABILITY
        t2 = IdentityType.MISCONFIGURATION
