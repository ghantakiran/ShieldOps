"""Tests for shieldops.security.security_workflow_optimizer — SecurityWorkflowOptimizer."""

from __future__ import annotations

from shieldops.security.security_workflow_optimizer import (
    SecurityWorkflowAnalysis,
    SecurityWorkflowLevel,
    SecurityWorkflowOptimizer,
    SecurityWorkflowRecord,
...
def _engine(**kw) -> SecurityWorkflowOptimizer:
    return SecurityWorkflowOptimizer(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert SecurityWorkflowType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert SecurityWorkflowType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert SecurityWorkflowType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert SecurityWorkflowType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert SecurityWorkflowType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert SecurityWorkflowSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert SecurityWorkflowSource.SIEM == "siem"

    def test_source_edr(self):
        assert SecurityWorkflowSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert SecurityWorkflowSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert SecurityWorkflowSource.MANUAL == "manual"

    def test_level_critical(self):
        assert SecurityWorkflowLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert SecurityWorkflowLevel.HIGH == "high"

    def test_level_medium(self):
        assert SecurityWorkflowLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert SecurityWorkflowLevel.LOW == "low"

    def test_level_info(self):
        assert SecurityWorkflowLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = SecurityWorkflowRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == SecurityWorkflowType.VULNERABILITY
        assert r.source == SecurityWorkflowSource.SCANNER
        assert r.level == SecurityWorkflowLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = SecurityWorkflowAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == SecurityWorkflowType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=SecurityWorkflowType.VULNERABILITY)
        e.record("b", record_type=SecurityWorkflowType.MISCONFIGURATION)
        results = e.list_records(record_type=SecurityWorkflowType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=SecurityWorkflowSource.SCANNER)
        e.record("b", source=SecurityWorkflowSource.SIEM)
        results = e.list_records(source=SecurityWorkflowSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = SecurityWorkflowType.VULNERABILITY
        t2 = SecurityWorkflowType.MISCONFIGURATION
