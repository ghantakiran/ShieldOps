"""Tests for shieldops.compliance.cyber_insurance_risk_assessor — CyberInsuranceRiskAssessor."""

from __future__ import annotations

from shieldops.compliance.cyber_insurance_risk_assessor import (
    CyberInsuranceAnalysis,
    CyberInsuranceLevel,
    CyberInsuranceRecord,
    CyberInsuranceReport,
...
def _engine(**kw) -> CyberInsuranceRiskAssessor:
    return CyberInsuranceRiskAssessor(**kw)


class TestEnums:
    def test_type_control(self):
        assert CyberInsuranceType.CONTROL == "control"

    def test_type_policy(self):
        assert CyberInsuranceType.POLICY == "policy"

    def test_type_regulation(self):
        assert CyberInsuranceType.REGULATION == "regulation"

    def test_type_standard(self):
        assert CyberInsuranceType.STANDARD == "standard"

    def test_type_framework(self):
        assert CyberInsuranceType.FRAMEWORK == "framework"

    def test_source_audit(self):
        assert CyberInsuranceSource.AUDIT == "audit"

    def test_source_automated_scan(self):
        assert CyberInsuranceSource.AUTOMATED_SCAN == "automated_scan"

    def test_source_manual_review(self):
        assert CyberInsuranceSource.MANUAL_REVIEW == "manual_review"

    def test_source_continuous_monitor(self):
        assert CyberInsuranceSource.CONTINUOUS_MONITOR == "continuous_monitor"

    def test_source_third_party(self):
        assert CyberInsuranceSource.THIRD_PARTY == "third_party"

    def test_level_compliant(self):
        assert CyberInsuranceLevel.COMPLIANT == "compliant"

    def test_level_partial(self):
        assert CyberInsuranceLevel.PARTIAL == "partial"

    def test_level_non_compliant(self):
        assert CyberInsuranceLevel.NON_COMPLIANT == "non_compliant"

    def test_level_not_assessed(self):
        assert CyberInsuranceLevel.NOT_ASSESSED == "not_assessed"

    def test_level_exempt(self):
        assert CyberInsuranceLevel.EXEMPT == "exempt"


class TestModels:
    def test_record_defaults(self):
        r = CyberInsuranceRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == CyberInsuranceType.CONTROL
        assert r.source == CyberInsuranceSource.AUDIT
        assert r.level == CyberInsuranceLevel.NON_COMPLIANT
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = CyberInsuranceAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == CyberInsuranceType.CONTROL
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=CyberInsuranceType.CONTROL)
        e.record("b", record_type=CyberInsuranceType.POLICY)
        results = e.list_records(record_type=CyberInsuranceType.CONTROL)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=CyberInsuranceSource.AUDIT)
        e.record("b", source=CyberInsuranceSource.AUTOMATED_SCAN)
        results = e.list_records(source=CyberInsuranceSource.AUDIT)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = CyberInsuranceType.CONTROL
        t2 = CyberInsuranceType.POLICY
