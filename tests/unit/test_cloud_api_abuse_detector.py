"""Tests for shieldops.security.cloud_api_abuse_detector — CloudAPIAbuseDetector."""

from __future__ import annotations

from shieldops.security.cloud_api_abuse_detector import (
    CloudApiAbuseAnalysis,
...
def _engine(**kw) -> CloudAPIAbuseDetector:
    return CloudAPIAbuseDetector(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert CloudApiAbuseType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert CloudApiAbuseType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert CloudApiAbuseType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert CloudApiAbuseType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert CloudApiAbuseType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert CloudApiAbuseSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert CloudApiAbuseSource.SIEM == "siem"

    def test_source_edr(self):
        assert CloudApiAbuseSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert CloudApiAbuseSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert CloudApiAbuseSource.MANUAL == "manual"

    def test_level_critical(self):
        assert CloudApiAbuseLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert CloudApiAbuseLevel.HIGH == "high"

    def test_level_medium(self):
        assert CloudApiAbuseLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert CloudApiAbuseLevel.LOW == "low"

    def test_level_info(self):
        assert CloudApiAbuseLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = CloudApiAbuseRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == CloudApiAbuseType.VULNERABILITY
        assert r.source == CloudApiAbuseSource.SCANNER
        assert r.level == CloudApiAbuseLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = CloudApiAbuseAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == CloudApiAbuseType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=CloudApiAbuseType.VULNERABILITY)
        e.record("b", record_type=CloudApiAbuseType.MISCONFIGURATION)
        results = e.list_records(record_type=CloudApiAbuseType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=CloudApiAbuseSource.SCANNER)
        e.record("b", source=CloudApiAbuseSource.SIEM)
        results = e.list_records(source=CloudApiAbuseSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = CloudApiAbuseType.VULNERABILITY
        t2 = CloudApiAbuseType.MISCONFIGURATION
