"""Tests for shieldops.security.oauth_token_abuse_detector — OAuthTokenAbuseDetector."""

from __future__ import annotations

from shieldops.security.oauth_token_abuse_detector import (
    OauthTokenAbuseAnalysis,
    OAuthTokenAbuseDetector,
    OauthTokenAbuseLevel,
    OauthTokenAbuseRecord,
    OauthTokenAbuseReport,
    OauthTokenAbuseSource,
    OauthTokenAbuseType,
)


def _engine(**kw) -> OAuthTokenAbuseDetector:
    return OAuthTokenAbuseDetector(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert OauthTokenAbuseType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert OauthTokenAbuseType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert OauthTokenAbuseType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert OauthTokenAbuseType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert OauthTokenAbuseType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert OauthTokenAbuseSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert OauthTokenAbuseSource.SIEM == "siem"

    def test_source_edr(self):
        assert OauthTokenAbuseSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert OauthTokenAbuseSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert OauthTokenAbuseSource.MANUAL == "manual"

    def test_level_critical(self):
        assert OauthTokenAbuseLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert OauthTokenAbuseLevel.HIGH == "high"

    def test_level_medium(self):
        assert OauthTokenAbuseLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert OauthTokenAbuseLevel.LOW == "low"

    def test_level_info(self):
        assert OauthTokenAbuseLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = OauthTokenAbuseRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == OauthTokenAbuseType.VULNERABILITY
        assert r.source == OauthTokenAbuseSource.SCANNER
        assert r.level == OauthTokenAbuseLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = OauthTokenAbuseAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == OauthTokenAbuseType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=OauthTokenAbuseType.VULNERABILITY)
        e.record("b", record_type=OauthTokenAbuseType.MISCONFIGURATION)
        results = e.list_records(record_type=OauthTokenAbuseType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=OauthTokenAbuseSource.SCANNER)
        e.record("b", source=OauthTokenAbuseSource.SIEM)
        results = e.list_records(source=OauthTokenAbuseSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = OauthTokenAbuseType.VULNERABILITY
        t2 = OauthTokenAbuseType.MISCONFIGURATION
