"""
Tests for shieldops.security.cross_platform_response_coordinator — CrossPlatformResponseCoordinator.
"""

from __future__ import annotations

from shieldops.security.cross_platform_response_coordinator import (
    CrossPlatformAnalysis,
    CrossPlatformLevel,
    CrossPlatformRecord,
    CrossPlatformReport,
    CrossPlatformResponseCoordinator,
    CrossPlatformSource,
    CrossPlatformType,
)


def _engine(**kw) -> CrossPlatformResponseCoordinator:
    return CrossPlatformResponseCoordinator(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert CrossPlatformType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert CrossPlatformType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert CrossPlatformType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert CrossPlatformType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert CrossPlatformType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert CrossPlatformSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert CrossPlatformSource.SIEM == "siem"

    def test_source_edr(self):
        assert CrossPlatformSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert CrossPlatformSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert CrossPlatformSource.MANUAL == "manual"

    def test_level_critical(self):
        assert CrossPlatformLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert CrossPlatformLevel.HIGH == "high"

    def test_level_medium(self):
        assert CrossPlatformLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert CrossPlatformLevel.LOW == "low"

    def test_level_info(self):
        assert CrossPlatformLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = CrossPlatformRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == CrossPlatformType.VULNERABILITY
        assert r.source == CrossPlatformSource.SCANNER
        assert r.level == CrossPlatformLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = CrossPlatformAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == CrossPlatformType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=CrossPlatformType.VULNERABILITY)
        e.record("b", record_type=CrossPlatformType.MISCONFIGURATION)
        results = e.list_records(record_type=CrossPlatformType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=CrossPlatformSource.SCANNER)
        e.record("b", source=CrossPlatformSource.SIEM)
        results = e.list_records(source=CrossPlatformSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = CrossPlatformType.VULNERABILITY
        t2 = CrossPlatformType.MISCONFIGURATION
