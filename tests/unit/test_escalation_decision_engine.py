"""Tests for shieldops.security.escalation_decision_engine — EscalationDecisionEngine."""

from __future__ import annotations

from shieldops.security.escalation_decision_engine import (
    EscalationAnalysis,
...
def _engine(**kw) -> EscalationDecisionEngine:
    return EscalationDecisionEngine(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert EscalationType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert EscalationType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert EscalationType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert EscalationType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert EscalationType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert EscalationSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert EscalationSource.SIEM == "siem"

    def test_source_edr(self):
        assert EscalationSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert EscalationSource.CLOUD_AUDIT == "cloud_audit"
    ...
    def test_level_info(self):
        assert EscalationLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == EscalationType.VULNERABILITY
        assert r.source == EscalationSource.SCANNER
        assert r.level == EscalationLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == EscalationType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=EscalationType.VULNERABILITY)
        e.record("b", record_type=EscalationType.MISCONFIGURATION)
        results = e.list_records(record_type=EscalationType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=EscalationSource.SCANNER)
        e.record("b", source=EscalationSource.SIEM)
        results = e.list_records(source=EscalationSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = EscalationType.VULNERABILITY
        t2 = EscalationType.MISCONFIGURATION
