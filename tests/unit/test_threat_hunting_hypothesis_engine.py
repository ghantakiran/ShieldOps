"""Tests for shieldops.security.threat_hunting_hypothesis_engine — ThreatHuntingHypothesisEngine."""

from __future__ import annotations

from shieldops.security.threat_hunting_hypothesis_engine import (
    ThreatHuntingAnalysis,
    ThreatHuntingHypothesisEngine,
    ThreatHuntingLevel,
    ThreatHuntingRecord,
    ThreatHuntingReport,
    ThreatHuntingSource,
...
def _engine(**kw) -> ThreatHuntingHypothesisEngine:
    return ThreatHuntingHypothesisEngine(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert ThreatHuntingType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert ThreatHuntingType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert ThreatHuntingType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert ThreatHuntingType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert ThreatHuntingType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert ThreatHuntingSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert ThreatHuntingSource.SIEM == "siem"

    def test_source_edr(self):
        assert ThreatHuntingSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert ThreatHuntingSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert ThreatHuntingSource.MANUAL == "manual"

    def test_level_critical(self):
        assert ThreatHuntingLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert ThreatHuntingLevel.HIGH == "high"

    def test_level_medium(self):
        assert ThreatHuntingLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert ThreatHuntingLevel.LOW == "low"

    def test_level_info(self):
        assert ThreatHuntingLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = ThreatHuntingRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == ThreatHuntingType.VULNERABILITY
        assert r.source == ThreatHuntingSource.SCANNER
        assert r.level == ThreatHuntingLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = ThreatHuntingAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == ThreatHuntingType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=ThreatHuntingType.VULNERABILITY)
        e.record("b", record_type=ThreatHuntingType.MISCONFIGURATION)
        results = e.list_records(record_type=ThreatHuntingType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=ThreatHuntingSource.SCANNER)
        e.record("b", source=ThreatHuntingSource.SIEM)
        results = e.list_records(source=ThreatHuntingSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = ThreatHuntingType.VULNERABILITY
        t2 = ThreatHuntingType.MISCONFIGURATION
