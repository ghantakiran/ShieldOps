"""
Tests for shieldops.analytics.security_investment_roi_analyzer — SecurityInvestmentROIAnalyzer.
"""

from __future__ import annotations

from shieldops.analytics.security_investment_roi_analyzer import (
    SecurityAnalysis,
...
def _engine(**kw) -> SecurityInvestmentROIAnalyzer:
    return SecurityInvestmentROIAnalyzer(**kw)


class TestEnums:
    def test_type_metric(self):
        assert SecurityType.METRIC == "metric"

    def test_type_trend(self):
        assert SecurityType.TREND == "trend"

    def test_type_forecast(self):
        assert SecurityType.FORECAST == "forecast"

    def test_type_anomaly(self):
        assert SecurityType.ANOMALY == "anomaly"

    def test_type_benchmark(self):
        assert SecurityType.BENCHMARK == "benchmark"

    def test_source_telemetry(self):
        assert SecuritySource.TELEMETRY == "telemetry"

    def test_source_log_analysis(self):
        assert SecuritySource.LOG_ANALYSIS == "log_analysis"

    def test_source_apm(self):
        assert SecuritySource.APM == "apm"

    def test_source_custom(self):
        assert SecuritySource.CUSTOM == "custom"

    def test_source_synthetic(self):
        assert SecuritySource.SYNTHETIC == "synthetic"
    ...
    def test_level_normal(self):
        assert SecurityLevel.NORMAL == "normal"

    def test_level_optimal(self):
        assert SecurityLevel.OPTIMAL == "optimal"

    def test_level_unknown(self):
        assert SecurityLevel.UNKNOWN == "unknown"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == SecurityType.METRIC
        assert r.source == SecuritySource.TELEMETRY
        assert r.level == SecurityLevel.NORMAL
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == SecurityType.METRIC
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=SecurityType.METRIC)
        e.record("b", record_type=SecurityType.TREND)
        results = e.list_records(record_type=SecurityType.METRIC)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=SecuritySource.TELEMETRY)
        e.record("b", source=SecuritySource.LOG_ANALYSIS)
        results = e.list_records(source=SecuritySource.TELEMETRY)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = SecurityType.METRIC
        t2 = SecurityType.TREND
