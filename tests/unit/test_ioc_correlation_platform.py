"""Tests for shieldops.security.ioc_correlation_platform — IOCCorrelationPlatform."""

from __future__ import annotations

from shieldops.security.ioc_correlation_platform import (
    IocCorrelationAnalysis,
    IocCorrelationLevel,
    IOCCorrelationPlatform,
    IocCorrelationRecord,
    IocCorrelationReport,
    IocCorrelationSource,
    IocCorrelationType,
)


def _engine(**kw) -> IOCCorrelationPlatform:
    return IOCCorrelationPlatform(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert IocCorrelationType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert IocCorrelationType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert IocCorrelationType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert IocCorrelationType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert IocCorrelationType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert IocCorrelationSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert IocCorrelationSource.SIEM == "siem"

    def test_source_edr(self):
        assert IocCorrelationSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert IocCorrelationSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert IocCorrelationSource.MANUAL == "manual"

    def test_level_critical(self):
        assert IocCorrelationLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert IocCorrelationLevel.HIGH == "high"

    def test_level_medium(self):
        assert IocCorrelationLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert IocCorrelationLevel.LOW == "low"

    def test_level_info(self):
        assert IocCorrelationLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = IocCorrelationRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == IocCorrelationType.VULNERABILITY
        assert r.source == IocCorrelationSource.SCANNER
        assert r.level == IocCorrelationLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = IocCorrelationAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == IocCorrelationType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=IocCorrelationType.VULNERABILITY)
        e.record("b", record_type=IocCorrelationType.MISCONFIGURATION)
        results = e.list_records(record_type=IocCorrelationType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=IocCorrelationSource.SCANNER)
        e.record("b", source=IocCorrelationSource.SIEM)
        results = e.list_records(source=IocCorrelationSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = IocCorrelationType.VULNERABILITY
        t2 = IocCorrelationType.MISCONFIGURATION
