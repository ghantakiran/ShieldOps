"""Tests for shieldops.operations.change_risk_auto_assessor — ChangeRiskAutoAssessor."""

from __future__ import annotations

from shieldops.operations.change_risk_auto_assessor import (
    ChangeRiskAutoAnalysis,
    ChangeRiskAutoAssessor,
    ChangeRiskAutoLevel,
    ChangeRiskAutoRecord,
    ChangeRiskAutoReport,
    ChangeRiskAutoSource,
    ChangeRiskAutoType,
)


def _engine(**kw) -> ChangeRiskAutoAssessor:
    return ChangeRiskAutoAssessor(**kw)


class TestEnums:
    def test_type_restart(self):
        assert ChangeRiskAutoType.RESTART == "restart"

    def test_type_scale(self):
        assert ChangeRiskAutoType.SCALE == "scale"

    def test_type_patch(self):
        assert ChangeRiskAutoType.PATCH == "patch"

    def test_type_rollback(self):
        assert ChangeRiskAutoType.ROLLBACK == "rollback"

    def test_type_config_change(self):
        assert ChangeRiskAutoType.CONFIG_CHANGE == "config_change"

    def test_source_monitoring(self):
        assert ChangeRiskAutoSource.MONITORING == "monitoring"

    def test_source_alert(self):
        assert ChangeRiskAutoSource.ALERT == "alert"

    def test_source_schedule(self):
        assert ChangeRiskAutoSource.SCHEDULE == "schedule"

    def test_source_manual(self):
        assert ChangeRiskAutoSource.MANUAL == "manual"

    def test_source_auto_detect(self):
        assert ChangeRiskAutoSource.AUTO_DETECT == "auto_detect"

    def test_level_critical(self):
        assert ChangeRiskAutoLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert ChangeRiskAutoLevel.HIGH == "high"

    def test_level_medium(self):
        assert ChangeRiskAutoLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert ChangeRiskAutoLevel.LOW == "low"

    def test_level_routine(self):
        assert ChangeRiskAutoLevel.ROUTINE == "routine"


class TestModels:
    def test_record_defaults(self):
        r = ChangeRiskAutoRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == ChangeRiskAutoType.RESTART
        assert r.source == ChangeRiskAutoSource.MONITORING
        assert r.level == ChangeRiskAutoLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = ChangeRiskAutoAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == ChangeRiskAutoType.RESTART
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=ChangeRiskAutoType.RESTART)
        e.record("b", record_type=ChangeRiskAutoType.SCALE)
        results = e.list_records(record_type=ChangeRiskAutoType.RESTART)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=ChangeRiskAutoSource.MONITORING)
        e.record("b", source=ChangeRiskAutoSource.ALERT)
        results = e.list_records(source=ChangeRiskAutoSource.MONITORING)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = ChangeRiskAutoType.RESTART
        t2 = ChangeRiskAutoType.SCALE
