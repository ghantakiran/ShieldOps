"""
Tests for shieldops.security.cloud_config_compliance_validator — CloudConfigComplianceValidator.
"""

from __future__ import annotations

from shieldops.security.cloud_config_compliance_validator import (
    CloudConfigAnalysis,
    CloudConfigComplianceValidator,
    CloudConfigLevel,
    CloudConfigRecord,
    CloudConfigReport,
    CloudConfigSource,
...
def _engine(**kw) -> CloudConfigComplianceValidator:
    return CloudConfigComplianceValidator(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert CloudConfigType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert CloudConfigType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert CloudConfigType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert CloudConfigType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert CloudConfigType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert CloudConfigSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert CloudConfigSource.SIEM == "siem"

    def test_source_edr(self):
        assert CloudConfigSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert CloudConfigSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert CloudConfigSource.MANUAL == "manual"

    def test_level_critical(self):
        assert CloudConfigLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert CloudConfigLevel.HIGH == "high"

    def test_level_medium(self):
        assert CloudConfigLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert CloudConfigLevel.LOW == "low"

    def test_level_info(self):
        assert CloudConfigLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == CloudConfigType.VULNERABILITY
        assert r.source == CloudConfigSource.SCANNER
        assert r.level == CloudConfigLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = CloudConfigAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == CloudConfigType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=CloudConfigType.VULNERABILITY)
        e.record("b", record_type=CloudConfigType.MISCONFIGURATION)
        results = e.list_records(record_type=CloudConfigType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=CloudConfigSource.SCANNER)
        e.record("b", source=CloudConfigSource.SIEM)
        results = e.list_records(source=CloudConfigSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = CloudConfigType.VULNERABILITY
        t2 = CloudConfigType.MISCONFIGURATION
