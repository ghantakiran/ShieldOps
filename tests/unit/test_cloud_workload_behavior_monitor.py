"""Tests for shieldops.security.cloud_workload_behavior_monitor — CloudWorkloadBehaviorMonitor."""

from __future__ import annotations

from shieldops.security.cloud_workload_behavior_monitor import (
    CloudWorkloadAnalysis,
    CloudWorkloadBehaviorMonitor,
    CloudWorkloadLevel,
    CloudWorkloadRecord,
    CloudWorkloadReport,
    CloudWorkloadSource,
...
def _engine(**kw) -> CloudWorkloadBehaviorMonitor:
    return CloudWorkloadBehaviorMonitor(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert CloudWorkloadType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert CloudWorkloadType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert CloudWorkloadType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert CloudWorkloadType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert CloudWorkloadType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert CloudWorkloadSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert CloudWorkloadSource.SIEM == "siem"

    def test_source_edr(self):
        assert CloudWorkloadSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert CloudWorkloadSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert CloudWorkloadSource.MANUAL == "manual"

    def test_level_critical(self):
        assert CloudWorkloadLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert CloudWorkloadLevel.HIGH == "high"

    def test_level_medium(self):
        assert CloudWorkloadLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert CloudWorkloadLevel.LOW == "low"

    def test_level_info(self):
        assert CloudWorkloadLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = CloudWorkloadRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == CloudWorkloadType.VULNERABILITY
        assert r.source == CloudWorkloadSource.SCANNER
        assert r.level == CloudWorkloadLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = CloudWorkloadAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == CloudWorkloadType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=CloudWorkloadType.VULNERABILITY)
        e.record("b", record_type=CloudWorkloadType.MISCONFIGURATION)
        results = e.list_records(record_type=CloudWorkloadType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=CloudWorkloadSource.SCANNER)
        e.record("b", source=CloudWorkloadSource.SIEM)
        results = e.list_records(source=CloudWorkloadSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = CloudWorkloadType.VULNERABILITY
        t2 = CloudWorkloadType.MISCONFIGURATION
