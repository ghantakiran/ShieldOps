"""Tests for CollectorConfigValidator."""

from __future__ import annotations

from shieldops.observability.collector_config_validator import (
...
def _engine(**kw) -> CollectorConfigValidator:
    return CollectorConfigValidator(**kw)


class TestEnums:
    def test_config_section_values(self):
        for v in ConfigSection:
...
class TestValidatePipelineConfig:
    def test_with_data(self):
        ...
        eng.add_record(
            name="a",
            config_section=ConfigSection.RECEIVERS,
            score=90.0,
        )
        result = eng.validate_pipeline_config()
        assert "receivers" in result

    def test_empty(self):
        eng = _engine()
        assert eng.validate_pipeline_config() == {}
...
class TestDetectConfigConflicts:
    def test_with_data(self):
        ...
        result = eng.detect_config_conflicts()
        assert len(result) == 1

    def test_empty(self):
        eng = _engine()
        assert eng.detect_config_conflicts() == []


class TestRecommendConfigFixes:
    def test_with_data(self):
        eng = _engine()
        eng.add_record(name="a", service="svc-a", score=30.0)
        result = eng.recommend_config_fixes()
        assert len(result) > 0

    def test_empty(self):
        eng = _engine()
        assert eng.recommend_config_fixes() == []
