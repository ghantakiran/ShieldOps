"""
Tests for shieldops.operations.remediation_verification_engine — RemediationVerificationEngine.
"""

from __future__ import annotations

from shieldops.operations.remediation_verification_engine import (
    RemediationAnalysis,
    RemediationLevel,
    RemediationRecord,
    RemediationReport,
...
def _engine(**kw) -> RemediationVerificationEngine:
    return RemediationVerificationEngine(**kw)
...
class TestEnums:
    ...
    def test_source_monitoring(self):
        assert RemediationSource.MONITORING == "monitoring"

    def test_source_alert(self):
        assert RemediationSource.ALERT == "alert"

    def test_source_schedule(self):
        assert RemediationSource.SCHEDULE == "schedule"

    def test_source_manual(self):
        assert RemediationSource.MANUAL == "manual"

    def test_source_auto_detect(self):
        assert RemediationSource.AUTO_DETECT == "auto_detect"

    def test_level_critical(self):
        assert RemediationLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert RemediationLevel.HIGH == "high"

    def test_level_medium(self):
        assert RemediationLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert RemediationLevel.LOW == "low"

    def test_level_routine(self):
        assert RemediationLevel.ROUTINE == "routine"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == RemediationType.RESTART
        assert r.source == RemediationSource.MONITORING
        assert r.level == RemediationLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == RemediationType.RESTART
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=RemediationType.RESTART)
        e.record("b", record_type=RemediationType.SCALE)
        results = e.list_records(record_type=RemediationType.RESTART)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=RemediationSource.MONITORING)
        e.record("b", source=RemediationSource.ALERT)
        results = e.list_records(source=RemediationSource.MONITORING)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = RemediationType.RESTART
        t2 = RemediationType.SCALE
