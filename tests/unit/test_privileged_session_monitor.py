"""Tests for shieldops.security.privileged_session_monitor — PrivilegedSessionMonitor."""

from __future__ import annotations

from shieldops.security.privileged_session_monitor import (
    PrivilegedAnalysis,
    PrivilegedLevel,
    PrivilegedRecord,
    PrivilegedReport,
    PrivilegedSessionMonitor,
    PrivilegedSource,
    PrivilegedType,
)


def _engine(**kw) -> PrivilegedSessionMonitor:
    return PrivilegedSessionMonitor(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert PrivilegedType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert PrivilegedType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert PrivilegedType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert PrivilegedType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert PrivilegedType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert PrivilegedSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert PrivilegedSource.SIEM == "siem"

    def test_source_edr(self):
        assert PrivilegedSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert PrivilegedSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert PrivilegedSource.MANUAL == "manual"

    def test_level_critical(self):
        assert PrivilegedLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert PrivilegedLevel.HIGH == "high"

    def test_level_medium(self):
        assert PrivilegedLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert PrivilegedLevel.LOW == "low"

    def test_level_info(self):
        assert PrivilegedLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == PrivilegedType.VULNERABILITY
        assert r.source == PrivilegedSource.SCANNER
        assert r.level == PrivilegedLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = PrivilegedAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == PrivilegedType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=PrivilegedType.VULNERABILITY)
        e.record("b", record_type=PrivilegedType.MISCONFIGURATION)
        results = e.list_records(record_type=PrivilegedType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=PrivilegedSource.SCANNER)
        e.record("b", source=PrivilegedSource.SIEM)
        results = e.list_records(source=PrivilegedSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = PrivilegedType.VULNERABILITY
        t2 = PrivilegedType.MISCONFIGURATION
