"""
Tests for shieldops.security.security_toolchain_health_monitor — SecurityToolchainHealthMonitor.
"""

from __future__ import annotations

from shieldops.security.security_toolchain_health_monitor import (
    SecurityAnalysis,
    SecurityLevel,
    SecurityRecord,
    SecurityReport,
    SecuritySource,
    SecurityToolchainHealthMonitor,
    SecurityType,
)


def _engine(**kw) -> SecurityToolchainHealthMonitor:
    return SecurityToolchainHealthMonitor(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert SecurityType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert SecurityType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert SecurityType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert SecurityType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert SecurityType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert SecuritySource.SCANNER == "scanner"

    def test_source_siem(self):
        assert SecuritySource.SIEM == "siem"

    def test_source_edr(self):
        assert SecuritySource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert SecuritySource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert SecuritySource.MANUAL == "manual"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == SecurityType.VULNERABILITY
        assert r.source == SecuritySource.SCANNER
        assert r.level == SecurityLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == SecurityType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=SecurityType.VULNERABILITY)
        e.record("b", record_type=SecurityType.MISCONFIGURATION)
        results = e.list_records(record_type=SecurityType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=SecuritySource.SCANNER)
        e.record("b", source=SecuritySource.SIEM)
        results = e.list_records(source=SecuritySource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = SecurityType.VULNERABILITY
        t2 = SecurityType.MISCONFIGURATION
