"""Tests for shieldops.audit.security_automation_audit_trail — SecurityAutomationAuditTrail."""

from __future__ import annotations

from shieldops.audit.security_automation_audit_trail import (
    SecurityAnalysis,
...
def _engine(**kw) -> SecurityAutomationAuditTrail:
    return SecurityAutomationAuditTrail(**kw)


class TestEnums:
    def test_type_control(self):
        assert SecurityType.CONTROL == "control"

    def test_type_policy(self):
        assert SecurityType.POLICY == "policy"

    def test_type_regulation(self):
        assert SecurityType.REGULATION == "regulation"

    def test_type_standard(self):
        assert SecurityType.STANDARD == "standard"

    def test_type_framework(self):
        assert SecurityType.FRAMEWORK == "framework"

    def test_source_audit(self):
        assert SecuritySource.AUDIT == "audit"

    def test_source_automated_scan(self):
        assert SecuritySource.AUTOMATED_SCAN == "automated_scan"

    def test_source_manual_review(self):
        assert SecuritySource.MANUAL_REVIEW == "manual_review"

    def test_source_continuous_monitor(self):
        assert SecuritySource.CONTINUOUS_MONITOR == "continuous_monitor"

    def test_source_third_party(self):
        assert SecuritySource.THIRD_PARTY == "third_party"

    def test_level_compliant(self):
        assert SecurityLevel.COMPLIANT == "compliant"

    def test_level_partial(self):
        assert SecurityLevel.PARTIAL == "partial"

    def test_level_non_compliant(self):
        assert SecurityLevel.NON_COMPLIANT == "non_compliant"

    def test_level_not_assessed(self):
        assert SecurityLevel.NOT_ASSESSED == "not_assessed"

    def test_level_exempt(self):
        assert SecurityLevel.EXEMPT == "exempt"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == SecurityType.CONTROL
        assert r.source == SecuritySource.AUDIT
        assert r.level == SecurityLevel.NON_COMPLIANT
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        ...
        assert a.id
        assert a.name == ""
        assert a.analysis_type == SecurityType.CONTROL
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=SecurityType.CONTROL)
        e.record("b", record_type=SecurityType.POLICY)
        results = e.list_records(record_type=SecurityType.CONTROL)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=SecuritySource.AUDIT)
        e.record("b", source=SecuritySource.AUTOMATED_SCAN)
        results = e.list_records(source=SecuritySource.AUDIT)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = SecurityType.CONTROL
        t2 = SecurityType.POLICY
