"""Tests for shieldops.security.authentication_bypass_detector — AuthenticationBypassDetector."""

from __future__ import annotations

from shieldops.security.authentication_bypass_detector import (
...
def _engine(**kw) -> AuthenticationBypassDetector:
    return AuthenticationBypassDetector(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert AuthenticationType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert AuthenticationType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert AuthenticationType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert AuthenticationType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert AuthenticationType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert AuthenticationSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert AuthenticationSource.SIEM == "siem"

    def test_source_edr(self):
        assert AuthenticationSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert AuthenticationSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert AuthenticationSource.MANUAL == "manual"

    def test_level_critical(self):
        assert AuthenticationLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert AuthenticationLevel.HIGH == "high"

    def test_level_medium(self):
        assert AuthenticationLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert AuthenticationLevel.LOW == "low"

    def test_level_info(self):
        assert AuthenticationLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = AuthenticationRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == AuthenticationType.VULNERABILITY
        assert r.source == AuthenticationSource.SCANNER
        assert r.level == AuthenticationLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = AuthenticationAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == AuthenticationType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=AuthenticationType.VULNERABILITY)
        e.record("b", record_type=AuthenticationType.MISCONFIGURATION)
        results = e.list_records(record_type=AuthenticationType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=AuthenticationSource.SCANNER)
        e.record("b", source=AuthenticationSource.SIEM)
        results = e.list_records(source=AuthenticationSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = AuthenticationType.VULNERABILITY
        t2 = AuthenticationType.MISCONFIGURATION
