"""Tests for shieldops.security.threat_landscape_mapper — ThreatLandscapeMapper."""

from __future__ import annotations

from shieldops.security.threat_landscape_mapper import (
    ThreatLandscapeAnalysis,
    ThreatLandscapeLevel,
    ThreatLandscapeMapper,
    ThreatLandscapeRecord,
    ThreatLandscapeReport,
    ThreatLandscapeSource,
    ThreatLandscapeType,
)


def _engine(**kw) -> ThreatLandscapeMapper:
    return ThreatLandscapeMapper(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert ThreatLandscapeType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert ThreatLandscapeType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert ThreatLandscapeType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert ThreatLandscapeType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert ThreatLandscapeType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert ThreatLandscapeSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert ThreatLandscapeSource.SIEM == "siem"

    def test_source_edr(self):
        assert ThreatLandscapeSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert ThreatLandscapeSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert ThreatLandscapeSource.MANUAL == "manual"

    def test_level_critical(self):
        assert ThreatLandscapeLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert ThreatLandscapeLevel.HIGH == "high"

    def test_level_medium(self):
        assert ThreatLandscapeLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert ThreatLandscapeLevel.LOW == "low"

    def test_level_info(self):
        assert ThreatLandscapeLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = ThreatLandscapeRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == ThreatLandscapeType.VULNERABILITY
        assert r.source == ThreatLandscapeSource.SCANNER
        assert r.level == ThreatLandscapeLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = ThreatLandscapeAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == ThreatLandscapeType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=ThreatLandscapeType.VULNERABILITY)
        e.record("b", record_type=ThreatLandscapeType.MISCONFIGURATION)
        results = e.list_records(record_type=ThreatLandscapeType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=ThreatLandscapeSource.SCANNER)
        e.record("b", source=ThreatLandscapeSource.SIEM)
        results = e.list_records(source=ThreatLandscapeSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = ThreatLandscapeType.VULNERABILITY
        t2 = ThreatLandscapeType.MISCONFIGURATION
