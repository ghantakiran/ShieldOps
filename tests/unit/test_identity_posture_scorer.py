"""Tests for shieldops.security.identity_posture_scorer — IdentityPostureScorer."""

from __future__ import annotations

from shieldops.security.identity_posture_scorer import (
    IdentityPostureAnalysis,
    IdentityPostureLevel,
    IdentityPostureRecord,
    IdentityPostureReport,
    IdentityPostureScorer,
    IdentityPostureSource,
    IdentityPostureType,
)


def _engine(**kw) -> IdentityPostureScorer:
    return IdentityPostureScorer(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert IdentityPostureType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert IdentityPostureType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert IdentityPostureType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert IdentityPostureType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert IdentityPostureType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert IdentityPostureSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert IdentityPostureSource.SIEM == "siem"

    def test_source_edr(self):
        assert IdentityPostureSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert IdentityPostureSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert IdentityPostureSource.MANUAL == "manual"

    def test_level_critical(self):
        assert IdentityPostureLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert IdentityPostureLevel.HIGH == "high"

    def test_level_medium(self):
        assert IdentityPostureLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert IdentityPostureLevel.LOW == "low"

    def test_level_info(self):
        assert IdentityPostureLevel.INFO == "info"


class TestModels:
    def test_record_defaults(self):
        r = IdentityPostureRecord()
        assert r.id
        assert r.name == ""
        assert r.record_type == IdentityPostureType.VULNERABILITY
        assert r.source == IdentityPostureSource.SCANNER
        assert r.level == IdentityPostureLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = IdentityPostureAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == IdentityPostureType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=IdentityPostureType.VULNERABILITY)
        e.record("b", record_type=IdentityPostureType.MISCONFIGURATION)
        results = e.list_records(record_type=IdentityPostureType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=IdentityPostureSource.SCANNER)
        e.record("b", source=IdentityPostureSource.SIEM)
        results = e.list_records(source=IdentityPostureSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = IdentityPostureType.VULNERABILITY
        t2 = IdentityPostureType.MISCONFIGURATION
