"""Tests for shieldops.security.serverless_security_scanner — ServerlessSecurityScanner."""

from __future__ import annotations

from shieldops.security.serverless_security_scanner import (
    ServerlessAnalysis,
    ServerlessLevel,
    ServerlessRecord,
    ServerlessReport,
    ServerlessSecurityScanner,
    ServerlessSource,
    ServerlessType,
)


def _engine(**kw) -> ServerlessSecurityScanner:
    return ServerlessSecurityScanner(**kw)


class TestEnums:
    def test_type_vulnerability(self):
        assert ServerlessType.VULNERABILITY == "vulnerability"

    def test_type_misconfiguration(self):
        assert ServerlessType.MISCONFIGURATION == "misconfiguration"

    def test_type_threat(self):
        assert ServerlessType.THREAT == "threat"

    def test_type_policy_violation(self):
        assert ServerlessType.POLICY_VIOLATION == "policy_violation"

    def test_type_anomaly(self):
        assert ServerlessType.ANOMALY == "anomaly"

    def test_source_scanner(self):
        assert ServerlessSource.SCANNER == "scanner"

    def test_source_siem(self):
        assert ServerlessSource.SIEM == "siem"

    def test_source_edr(self):
        assert ServerlessSource.EDR == "edr"

    def test_source_cloud_audit(self):
        assert ServerlessSource.CLOUD_AUDIT == "cloud_audit"

    def test_source_manual(self):
        assert ServerlessSource.MANUAL == "manual"

    def test_level_critical(self):
        assert ServerlessLevel.CRITICAL == "critical"

    def test_level_high(self):
        assert ServerlessLevel.HIGH == "high"

    def test_level_medium(self):
        assert ServerlessLevel.MEDIUM == "medium"

    def test_level_low(self):
        assert ServerlessLevel.LOW == "low"

    def test_level_info(self):
        assert ServerlessLevel.INFO == "info"
...
class TestModels:
    def test_record_defaults(self):
        ...
        assert r.id
        assert r.name == ""
        assert r.record_type == ServerlessType.VULNERABILITY
        assert r.source == ServerlessSource.SCANNER
        assert r.level == ServerlessLevel.MEDIUM
        assert r.score == 0.0
        assert r.service == ""
        assert r.team == ""
    ...
    def test_analysis_defaults(self):
        a = ServerlessAnalysis()
        assert a.id
        assert a.name == ""
        assert a.analysis_type == ServerlessType.VULNERABILITY
...
class TestRecord:
    ...
    def test_list_records_filter_type(self):
        e = _engine()
        e.record("a", record_type=ServerlessType.VULNERABILITY)
        e.record("b", record_type=ServerlessType.MISCONFIGURATION)
        results = e.list_records(record_type=ServerlessType.VULNERABILITY)
        assert len(results) == 1

    def test_list_records_filter_source(self):
        e = _engine()
        e.record("a", source=ServerlessSource.SCANNER)
        e.record("b", source=ServerlessSource.SIEM)
        results = e.list_records(source=ServerlessSource.SCANNER)
        assert len(results) == 1
...
class TestDistribution:
    ...
    def test_multiple_types(self):
        e = _engine()
        t1 = ServerlessType.VULNERABILITY
        t2 = ServerlessType.MISCONFIGURATION
